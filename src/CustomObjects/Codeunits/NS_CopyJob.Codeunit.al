/// <summary>
/// Codeunit NS Copy Job (ID 14021190).
/// </summary>
//PRJ-1217.GK.1.0 16mar2022-Add New codeunit
codeunit 14021190 "NS Copy Job"
{
    ...
    begin
        ...
        if SourceJobTask.FindSet then
            repeat
                ...
                TargetJobTask.SetRange("Job No.", TargetJob."No.");
                TargetJobTask.SetRange("Job Task No.", SourceJobTask."Job Task No.");
                if not TargetJobTask.FindFirst() then begin
                    TargetJobTask.Init();
                    ...
                    case true of
                        JobPlanningLineSource = JobPlanningLineSource::"Job Planning Lines":
                            UpdateJobPlanningLines(SourceJobTask, TargetJobTask);//PRJ-1217.GK.1.0
                                                                                 // JobPlanningLineSource = JobPlanningLineSource::"Job Ledger Entries":
                                                                                 //     CopyJLEsToJobPlanningLines(SourceJobTask, TargetJobTask);
                    end;
                end;
    ...
    //PRJ-1217.GK.1.0 16mar2022 start
    procedure UpdateJobPlanningLines(SourceJobTask: Record "Job Task"; TargetJobTask: Record "Job Task")
    var
        SourceJobPlanningLine: Record "Job Planning Line";
        TargetJobPlanningLine: Record "Job Planning Line";
        SourceJob: Record Job;
        ...
        if SourceJobPlanningLine.FindSet then
            repeat
                with TargetJobPlanningLine do begin
                    ...
                    TargetJobPlanningLine.SetRange("Job Task No.", SourceJobTask."Job Task No.");
                    TargetJobPlanningLine.SetRange("Line No.", SourceJobPlanningLine."Line No.");
                    if not TargetJobPlanningLine.FindFirst() then begin
                        //PRJ-1217 end
                        Init;
                        Validate("Job No.", TargetJobTask."Job No.");
                        Validate("Job Task No.", TargetJobTask."Job Task No.");
