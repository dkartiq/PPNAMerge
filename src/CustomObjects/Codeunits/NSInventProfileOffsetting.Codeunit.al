codeunit 14021117 "NS_Invent. Profile Offsetting"
{
    ...
    // +         - Set JobOnly global
    // +     - TransPurchLineToProfile:
    // +         - Pass JobOnly to purchase line
    //PRJ-130.MS.1.0 added code for flow of vendor and manu. from JMP to Plan.Line
    ...
    begin
        //ProjectPro. Added params: JobDemandOnly, VAR JobNoFilter, DocumentNo

        NS_OnBeforeCalculatePlanFromWorksheet(
          Item, ManufacturingSetup2, TemplateName, WorksheetName, OrderDate, ToDate, MRPPlanning, RespectPlanningParm);

        PlanToDate := ToDate;
        NS_InitVariables(InventoryProfile[1], ManufacturingSetup2, Item, TemplateName, WorksheetName, MRPPlanning);

        //ProjectPro - start
        //DemandtoInvProfile(InventoryProfile[1],Item,ToDate);
        JobOnly := JobDemandOnly;
        NS_DemandToInvtProfile(InventoryProfile[1], Item, ToDate, JobDemandOnly, JobNoFilter);
        //ProjectPro - end

        OrderDate := NS_ForecastConsumption(InventoryProfile[1], Item, OrderDate, ToDate);
        NS_BlanketOrderConsump(InventoryProfile[1], Item, ToDate);
        NS_SupplytoInvProfile(InventoryProfile[1], Item, ToDate);
        NS_UnfoldItemTracking(InventoryProfile[1], InventoryProfile[2]);
        NS_FindCombination(InventoryProfile[1], InventoryProfile[2], Item);

        //ProjectPro - start
        if not JobDemandOnly then
            NS_PlanItem(InventoryProfile[1], InventoryProfile[2], OrderDate, ToDate, RespectPlanningParm) //Original Line
        else
            NS_AddJobDemandOnly(InventoryProfile[1], Item, ToDate, JobDemandOnly, JobNoFilter, OrderDate, DocumentNo);
        //ProjectPro - end

        NS_CommitTracking;
    end;

    local procedure NS_InitVariables(var InventoryProfile: Record "Inventory Profile"; ManufacturingSetup2: Record "Manufacturing Setup"; Item: Record Item; TemplateName: Code[10]; WorksheetName: Code[10]; MRPPlanning: Boolean)
    var
    ...
    local procedure NS_CreateTempSKUForLocation(ItemNo: Code[20]; LocationCode: Code[10])
    begin
        TempSKU.Init;
        TempSKU."Item No." := ItemNo;
        NS_TransferPlanningParameters(TempSKU);
        TempSKU."Location Code" := LocationCode;
        TempSKU.Insert;
    end;

    local procedure NS_DemandToInvtProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; ToDate: Date; JobDemandOnly: Boolean; var JobNoFilter: Code[20])
    var
        CopyOfItem: Record Item;
    begin
        //ProjectPro. Added params: ;JobDemandOnly@1100773001 : Boolean;VAR JobNoFilter@1100773000 : Code[20]
        InventoryProfile.SetCurrentKey("Line No.");

        CopyOfItem.Copy(Item);
        Item.SetRange("Date Filter", 0D, ToDate);

        NS_TransSalesLineToProfile(InventoryProfile, Item);
        NS_TransServLineToProfile(InventoryProfile, Item);

        //ProjectPro - start
        //TransJobPlanningLineToProfile(InventoryProfile,Item);
        NS_TransJobPlanningLineToProfile(InventoryProfile, Item, JobNoFilter);
        //ProjectPro - end

        NS_TransProdOrderCompToProfile(InventoryProfile, Item);
        NS_TransAsmLineToProfile(InventoryProfile, Item);
        NS_TransPlanningCompToProfile(InventoryProfile, Item);
        NS_TransTransReqLineToProfile(InventoryProfile, Item, ToDate);
        NS_TransShptTransLineToProfile(InventoryProfile, Item);

        NS_OnAfterDemandToInvProfile(InventoryProfile, Item, TempItemTrkgEntry, LineNo);

        Item.Copy(CopyOfItem);
    end;

    local procedure NS_SupplytoInvProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; ToDate: Date)
    var
        CopyOfItem: Record Item;
    begin
        InventoryProfile.Reset;
        ...
        NS_OnBeforeSupplyToInvProfile(InventoryProfile, Item, ToDate, TempItemTrkgEntry, LineNo);

        NS_TransItemLedgEntryToProfile(InventoryProfile, Item);
        NS_TransReqLineToProfile(InventoryProfile, Item, ToDate);
        NS_TransPurchLineToProfile(InventoryProfile, Item, ToDate);
        NS_TransProdOrderToProfile(InventoryProfile, Item, ToDate);
        NS_TransAsmHeaderToProfile(InventoryProfile, Item, ToDate);
        NS_TransRcptTransLineToProfile(InventoryProfile, Item, ToDate);

        NS_OnAfterSupplyToInvProfile(InventoryProfile, Item, ToDate, TempItemTrkgEntry, LineNo);

        Item.Copy(CopyOfItem);
    end;

    local procedure NS_InsertSupplyProfile(var InventoryProfile: Record "Inventory Profile"; ToDate: Date)
    begin
    ...
    local procedure NS_TransSalesLineToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item)
    var
        SalesLine: Record "Sales Line";
    begin

        if SalesLine.FindLinesWithItemToPlan(Item, SalesLine."Document Type"::Order.AsInteger()) then
            repeat
                if SalesLine."Shipment Date" <> 0D then begin
                    InventoryProfile.Init;
                    InventoryProfile."Line No." := NS_NextLineNo;
        ...
        if SalesLine.FindLinesWithItemToPlan(Item, SalesLine."Document Type"::"Return Order".AsInteger()) then
            repeat
                if SalesLine."Shipment Date" <> 0D then begin
                    InventoryProfile.Init;
                    InventoryProfile."Line No." := NS_NextLineNo;
    ...
    local procedure NS_TransServLineToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item)
    var
    ...
    local procedure NS_TransJobPlanningLineToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; var JobNoFilter: Code[20])
    var
        JobPlanningLine: Record "Job Planning Line";
    begin
        //ProjectPro. Added param: VAR JobNoFilter
        if JobPlanningLine.FindLinesWithItemToPlan(Item) then
            repeat
                ...
                if (JobNoFilter <> '') and (JobNoFilter = JobPlanningLine."Job No.") then begin
                    if JobPlanningLine."Planning Date" <> 0D then begin
                        InventoryProfile.Init;
                        InventoryProfile."Line No." := NS_NextLineNo;
                        InventoryProfile.TransferFromJobPlanningLine(JobPlanningLine, TempItemTrkgEntry);
    ...
    local procedure NS_TransProdOrderCompToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item)
    var
    ...
    local procedure NS_TransPlanningCompToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item)
    var
    ...
    local procedure NS_TransAsmLineToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item)
    var
            ...
            repeat
                if AsmLine."Due Date" <> 0D then begin
                    ...
                    if not ReqLine.FindFirst then
                        NS_InsertAsmLineToProfile(InventoryProfile, AsmLine, 1);
                end;
            ...
            repeat
                if AsmLine."Due Date" <> 0D then begin
                    ...
                    if not ReqLine.FindFirst then begin
                        ...
                        RemRatio := (AsmHeader."Quantity (Base)" - NS_CalcSalesOrderQty(AsmLine)) / AsmHeader."Quantity (Base)";
                        NS_InsertAsmLineToProfile(InventoryProfile, AsmLine, RemRatio);
                    end;
                end;
            until AsmLine.Next = 0;
    end;

    local procedure NS_TransTransReqLineToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; ToDate: Date)
    var
    ...
    local procedure NS_TransShptTransLineToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item)
    var
            ...
            repeat
                if TransLine."Shipment Date" <> 0D then begin
                    ...
                    if FilterIsSetOnLocation then
                        InventoryProfile."Transfer Location Not Planned" := NS_TransferLocationIsFilteredOut(Item, TransLine);
                    NS_SyncTransferDemandWithReqLine(InventoryProfile, TransLine."Transfer-to Code");
                    InventoryProfile.Insert;
                end;
            until TransLine.Next = 0;
    end;

    local procedure NS_TransItemLedgEntryToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item)
    begin
    ...
    local procedure NS_TransReqLineToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; ToDate: Date)
    var
        ReqLine: Record "Requisition Line";
    begin
        if ReqLine.FindLinesWithItemToPlan(Item) then
            repeat
                if ReqLine."Due Date" <> 0D then begin
                    ...
                    NS_InsertSupplyProfile(InventoryProfile, ToDate);
                end;
            until ReqLine.Next = 0;
    end;

    local procedure NS_TransPurchLineToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; ToDate: Date)
    var
        PurchLine: Record "Purchase Line";
    begin
        //ProjectPro - start
        PurchLine.SetJobDemandOnly(JobOnly);
        //ProjectPro - end

        if PurchLine.FindLinesWithItemToPlan(Item, PurchLine."Document Type"::Order.AsInteger()) then
            repeat
                if PurchLine."Expected Receipt Date" <> 0D then
                    if PurchLine."Prod. Order No." = '' then
                        NS_InsertPurchLineToProfile(InventoryProfile, PurchLine, ToDate);
            until PurchLine.Next = 0;

        if PurchLine.FindLinesWithItemToPlan(Item, PurchLine."Document Type"::"Return Order".AsInteger()) then
            repeat
                if PurchLine."Expected Receipt Date" <> 0D then
                    if PurchLine."Prod. Order No." = '' then
                        NS_InsertPurchLineToProfile(InventoryProfile, PurchLine, ToDate);
    ...
    local procedure NS_TransProdOrderToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; ToDate: Date)
    var
    ...
    local procedure NS_TransAsmHeaderToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; ToDate: Date)
    var
        AsmHeader: Record "Assembly Header";
    begin
    ...
    local procedure NS_TransRcptTransLineToProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; ToDate: Date)
    var
        TransLine: Record "Transfer Line";
            ...
            repeat
                if TransLine."Receipt Date" <> 0D then begin
                    ...
                    if FilterIsSetOnLocation then
                        InventoryProfile."Transfer Location Not Planned" := NS_TransferLocationIsFilteredOut(Item, TransLine);
                    NS_InsertSupplyProfile(InventoryProfile, ToDate);
                    NS_InsertTempTransferSKU(TransLine);
                end;
            until TransLine.Next = 0;
    end;

    local procedure NS_TransferLocationIsFilteredOut(var Item: Record Item; var TransLine: Record "Transfer Line"): Boolean
    var
    ...
    local procedure NS_InsertPurchLineToProfile(var InventoryProfile: Record "Inventory Profile"; PurchLine: Record "Purchase Line"; ToDate: Date)
    begin
        InventoryProfile.Init;
        InventoryProfile."Line No." := NS_NextLineNo;
    ...
    local procedure NS_InsertAsmLineToProfile(var InventoryProfile: Record "Inventory Profile"; AsmLine: Record "Assembly Line"; RemRatio: Decimal)
    begin
        InventoryProfile.Init;
        InventoryProfile."Line No." := NS_NextLineNo;
    ...
    local procedure NS_ForecastConsumption(var DemandInvtProfile: Record "Inventory Profile"; var Item: Record Item; OrderDate: Date; ToDate: Date) UpdatedOrderDate: Date
    var
    ...
    begin
        ...
        if not ManufacturingSetup."Use Forecast on Locations" then begin
            ReplenishmentLocationFound := NS_FindReplishmentLocation(ReplenishmentLocation, Item);
        ...
        for ComponentForecast := ComponentForecastFrom to true do begin
            ...
            if ForecastEntry2.Find('-') then
                repeat
                    ...
                    ForecastExist := NS_CheckForecastExist(ForecastEntry, OrderDate, ToDate);
                    ...
                    if ForecastExist then
                        repeat
                            ...
                            if TotalForecastQty > 0 then begin
                                NS_ForecastInitDemand(DemandInvtProfile, ForecastEntry, Item."No.", ReplenishmentLocation, TotalForecastQty);
    ...
    local procedure NS_BlanketOrderConsump(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; ToDate: Date)
    var
    ...
    begin
        ...
        with BlanketSalesLine do begin
            ...
            NS_OnBeforeBlanketOrderConsumpFind(BlanketSalesLine);
            if Find('-') then
                repeat
                    QtyReleased += NS_CalcInventoryProfileRemainingQty(InventoryProfile, "Document No.");
                    SetRange("Document No.", "Document No.");
                    NS_BlanketSalesOrderLinesToProfile(InventoryProfile, BlanketSalesLine, QtyReleased);
                    SetRange("Document No.");
                until Next = 0;
        end;
    end;

    local procedure NS_BlanketSalesOrderLinesToProfile(var InventoryProfile: Record "Inventory Profile"; var BlanketSalesLine: Record "Sales Line"; var QtyReleased: Decimal)
    var
            ...
            for IsSalesOrderLineCreated := true downto false do begin
                ...
                repeat
                    if "Quantity (Base)" <> "Qty. to Asm. to Order (Base)" then
                        if NS_DoProcessBlanketLine("Document No.", "Line No.", IsSalesOrderLineCreated) then
    ...
    local procedure NS_DoProcessBlanketLine(BlanketOrderNo: Code[20]; BlanketOrderLineNo: Integer; IsSalesOrderLineCreated: Boolean): Boolean
    var
        SalesLine: Record "Sales Line";
    begin
    ...
    local procedure NS_CheckForecastExist(var ForecastEntry: Record "Production Forecast Entry"; OrderDate: Date; ToDate: Date): Boolean
    var
        ForecastExist: Boolean;
    begin
    ...
    local procedure NS_FindReplishmentLocation(var ReplenishmentLocation: Code[10]; var Item: Record Item): Boolean
    var
    ...
    local procedure NS_FindCombination(var DemandInvtProfile: Record "Inventory Profile"; var SupplyInvtProfile: Record "Inventory Profile"; var Item: Record Item)
    var
        SKU: Record "Stockkeeping Unit";
    ...
    begin
        NS_CreateTempSKUForComponentsLocation(Item);
        ...
        while DemandBool or SupplyBool do begin
            if DemandBool then begin
                ...
                NS_OnFindCombinationAfterAssignTempSKU(TempSKU, DemandInvtProfile);
            end else begin
                ...
                NS_OnFindCombinationAfterAssignTempSKU(TempSKU, SupplyInvtProfile);
            end;
            ...
            case State of
                State::DemandExist:
                    DemandBool := NS_FindNextSKU(DemandInvtProfile);
                State::SupplyExist:
                    SupplyBool := NS_FindNextSKU(SupplyInvtProfile);
                State::BothExist:
    ...
    local procedure NS_InsertTempSKU()
    var
    ...
    local procedure NS_FindNextSKU(var InventoryProfile: Record "Inventory Profile"): Boolean
    begin
    ...
    local procedure NS_TransferPlanningParameters(var SKU: Record "Stockkeeping Unit")
    var
        SKU2: Record "Stockkeeping Unit";
        PlanningGetParameters: Codeunit "Planning-Get Parameters";
    begin
    ...
    local procedure NS_DeleteTracking(var SKU: Record "Stockkeeping Unit"; ToDate: Date; var SupplyInventoryProfile: Record "Inventory Profile")
    var
        Item: Record Item;
    ...
    begin
        ...
        with ReservEntry do begin
            ...
            if Find('-') then
                repeat
                    Item.Get("Item No.");
                    if not NS_IsTrkgForSpecialOrderOrDropShpt(ReservEntry) then begin
                        if NS_ShouldDeleteReservEntry(ReservEntry, ToDate) then begin
                            ...
                            ResEntryWasDeleted := NS_CloseTracking(ReservEntry, SupplyInventoryProfile, ToDate);
    ...
    local procedure NS_ShouldDeleteReservEntry(ReservEntry: Record "Reservation Entry"; ToDate: Date): Boolean
    var
        Item: Record Item;
        IsReservedForProdComponent: Boolean;
    begin
        IsReservedForProdComponent := NS_ReservedForProdComponent(ReservEntry);

        if IsReservedForProdComponent and NS_IsProdOrderPlanned(ReservEntry) and
           (ReservEntry."Reservation Status".AsInteger() > ReservEntry."Reservation Status"::Tracking.AsInteger())
        then
            exit(false);
    ...
    local procedure NS_IsProdOrderPlanned(ReservationEntry: Record "Reservation Entry"): Boolean
    var
        ProdOrderComp: Record "Prod. Order Component";
    ...
    local procedure NS_RemoveOrdinaryInventory(var Supply: Record "Inventory Profile")
    var
    ...
    local procedure NS_UnfoldItemTracking(var ParentInvProfile: Record "Inventory Profile"; var ChildInvProfile: Record "Inventory Profile")
    begin
            ...
            repeat
                ...
                if TempItemTrkgEntry.Find('-') then begin
                    ...
                    repeat
                        ...
                        ChildInvProfile."Line No." := NS_NextLineNo;
                        ...
                        NS_OnAfterTransToChildInvProfile(TempItemTrkgEntry, ChildInvProfile);
    ...
    local procedure NS_MatchAttributes(var SupplyInvtProfile: Record "Inventory Profile"; var DemandInvtProfile: Record "Inventory Profile"; RespectPlanningParm: Boolean)
    var
    ...
    begin
        ...
        ItemInventoryExists := NS_CheckItemInventoryExists(SupplyInvtProfile);
        ...
        if DemandInvtProfile.FindSet(true) then
            repeat
                ...
                NS_OnBeforeMatchAttributesDemandApplicationLoop(SupplyInvtProfile, DemandInvtProfile, SupplyExists);
                while (DemandInvtProfile."Untracked Quantity" > 0) and
                      (not NS_ApplyUntrackedQuantityToItemInventory(SupplyExists, ItemInventoryExists))
                do begin
                    NS_OnStartOfMatchAttributesDemandApplicationLoop(SupplyInvtProfile, DemandInvtProfile, SupplyExists);
                    if SupplyExists and (DemandInvtProfile.Binding = DemandInvtProfile.Binding::"Order-to-Order") then begin
                        ...
                        if CanBeRescheduled then
                            if (SupplyInvtProfile."Due Date" > DemandInvtProfile."Due Date") or
                               (SupplyInvtProfile."Due Time" > DemandInvtProfile."Due Time")
                            then
                                CanBeRescheduled := NS_CheckScheduleIn(SupplyInvtProfile, DemandInvtProfile."Due Date", NewSupplyDate, false)
                            else
                                CanBeRescheduled := NS_CheckScheduleOut(SupplyInvtProfile, DemandInvtProfile."Due Date", NewSupplyDate, false);
                        if CanBeRescheduled and
                           ((NewSupplyDate <> SupplyInvtProfile."Due Date") or (SupplyInvtProfile."Planning Level Code" > 0))
                        then begin
                            NS_Reschedule(SupplyInvtProfile, DemandInvtProfile."Due Date", DemandInvtProfile."Due Time");
                    ...
                    if not SupplyExists or (SupplyInvtProfile."Due Date" > DemandInvtProfile."Due Date") then begin
                        NS_InitSupply(SupplyInvtProfile, DemandInvtProfile."Untracked Quantity", DemandInvtProfile."Due Date");
                        NS_TransferAttributes(SupplyInvtProfile, DemandInvtProfile);
                        SupplyInvtProfile."Fixed Date" := SupplyInvtProfile."Due Date";
                    ...
                    if DemandInvtProfile.Binding = DemandInvtProfile.Binding::"Order-to-Order" then
                        ...
                        then
                            NS_IncreaseQtyToMeetDemand(SupplyInvtProfile, DemandInvtProfile, false, RespectPlanningParm, false);

                    if SupplyInvtProfile."Untracked Quantity" < DemandInvtProfile."Untracked Quantity" then
                        SupplyExists := NS_CloseSupply(DemandInvtProfile, SupplyInvtProfile)
                    else
                        NS_CloseDemand(DemandInvtProfile, SupplyInvtProfile);
                    NS_OnEndMatchAttributesDemandApplicationLoop(SupplyInvtProfile, DemandInvtProfile, SupplyExists);
                end;
        ...
        if SupplyInvtProfile.FindSet then
            repeat
                if SupplyInvtProfile."Untracked Quantity" > 0 then begin
                    if NS_DecreaseQty(SupplyInvtProfile, SupplyInvtProfile."Untracked Quantity") then begin
                        ...
                        NS_MaintainPlanningLine(SupplyInvtProfile, PlanningLineStage::Exploded, ScheduleDirection::Backward)
                    end else begin
                        ...
                        if TempSKU."Include Inventory" then begin
                            ...
                            NS_SetAttributePriority(SupplyInvtProfile);
                        end else
                    ...
                    if SupplyInvtProfile."Untracked Quantity" <> 0 then begin
                        NS_UpdateQty(SupplyInvtProfile, SupplyInvtProfile."Untracked Quantity");
    ...
    local procedure NS_MatchReservationEntries(var FromTrkgReservEntry: Record "Reservation Entry"; var ToTrkgReservEntry: Record "Reservation Entry")
    begin
        ...
        then begin
            NS_SwitchTrackingToReservationStatus(FromTrkgReservEntry);
            NS_SwitchTrackingToReservationStatus(ToTrkgReservEntry);
        end;
    end;

    local procedure NS_SwitchTrackingToReservationStatus(var ReservEntry: Record "Reservation Entry")
    begin
    ...
    local procedure NS_PlanItem(var DemandInvtProfile: Record "Inventory Profile"; var SupplyInvtProfile: Record "Inventory Profile"; PlanningStartDate: Date; ToDate: Date; RespectPlanningParm: Boolean)
    var
    ...
    begin
        ...
        NS_UpdateTempSKUTransferLevels;
        ...
        if TempSKU.Find('-') then
            repeat
                ...
                NS_FilterDemandSupplyRelatedToSKU(DemandInvtProfile);
                NS_FilterDemandSupplyRelatedToSKU(SupplyInvtProfile);
                ...
                if not TempSKU."Include Inventory" then
                    NS_RemoveOrdinaryInventory(SupplyInvtProfile);
                NS_InsertSafetyStockDemands(DemandInvtProfile, PlanningStartDate);
                NS_UpdatePriorities(SupplyInvtProfile, IsReorderPointPlanning, ToDate);
                ...
                PlanThisSKU := NS_CheckPlanSKU(TempSKU, DemandExists, SupplyExists, IsReorderPointPlanning);

                if PlanThisSKU then begin
                    NS_PrepareDemand(DemandInvtProfile, IsReorderPointPlanning, ToDate);
                    PlanThisSKU :=
                      not (NS_DemandMatchedSupply(DemandInvtProfile, SupplyInvtProfile, TempSKU) and
                           NS_DemandMatchedSupply(SupplyInvtProfile, DemandInvtProfile, TempSKU));
                end;
                if PlanThisSKU then begin
                    // Preliminary clean of tracking
                    if DemandExists or SupplyExists then
                        NS_DeleteTracking(TempSKU, ToDate, SupplyInvtProfile);

                    NS_MatchAttributes(SupplyInvtProfile, DemandInvtProfile, RespectPlanningParm);

                    // Calculate initial inventory
                    NS_PlanItemCalcInitialInventory(
                      DemandInvtProfile, SupplyInvtProfile, PlanningStartDate, DemandExists, SupplyExists, LastProjectedInventory);

                    NS_OnBeforePrePlanDateDemandProc(SupplyInvtProfile, DemandInvtProfile, SupplyExists, DemandExists);
                    ...
                    NS_OnBeforePrePlanDateSupplyProc(SupplyInvtProfile, DemandInvtProfile, SupplyExists, DemandExists);
                    ...
                    NS_OnAfterPrePlanDateSupplyProc(SupplyInvtProfile, DemandInvtProfile, SupplyExists, DemandExists);

                    if LastAvailableInventory < 0 then begin // Emergency order
                                                             // Insert Supply
                        NS_InitSupply(SupplyInvtProfile, -LastAvailableInventory, PlanningStartDate - 1);
                        SupplyInvtProfile."Planning Flexibility" := SupplyInvtProfile."Planning Flexibility"::None;
                        SupplyInvtProfile.Insert;
                        NS_MaintainPlanningLine(SupplyInvtProfile, PlanningLineStage::Exploded, ScheduleDirection::Backward);
                        NS_Track(SupplyInvtProfile, DemandInvtProfile, true, false, SupplyInvtProfile.Binding::" ".AsInteger());
                        LastProjectedInventory += SupplyInvtProfile."Remaining Quantity (Base)";
                        LastAvailableInventory += SupplyInvtProfile."Untracked Quantity";
                    ...
                    if LastAvailableInventory < TempSKU."Safety Stock Quantity" then begin // Initial Safety Stock Warning
                        SupplyAvailableWithinLeadTime := NS_SumUpAvailableSupply(SupplyInvtProfile, PlanningStartDate, PlanningStartDate);
                        ...
                        if InitialProjectedInventory < TempSKU."Safety Stock Quantity" then
                            NS_CreateSupplyForInitialSafetyStockWarning(
                              SupplyInvtProfile,
                    ...
                    if IsReorderPointPlanning then begin
                        SupplyWithinLeadtime :=
                          NS_SumUpProjectedSupply(SupplyInvtProfile, PlanningStartDate, PlanningStartDate + BucketSizeInDays - 1);

                        if LastProjectedInventory + SupplyWithinLeadtime <= TempSKU."Reorder Point" then begin
                            NS_InitSupply(SupplyInvtProfile, 0, 0D);
                            NS_CreateSupplyForward(
                    ...
                    if IsReorderPointPlanning then
                        NS_ChkInitialOverflow(DemandInvtProfile, SupplyInvtProfile,
                          OverflowLevel, LastProjectedInventory, PlanningStartDate, ToDate);

                    NS_CheckSupplyWithSKU(SupplyInvtProfile, TempSKU);
                        ...
                        case NextState of
                            NextState::StartOver:
                                begin
                                    if DemandExists and (DemandInvtProfile."Source Type" = DATABASE::"Transfer Line") then
                                        while NS_CancelTransfer(SupplyInvtProfile, DemandInvtProfile, DemandExists) do
                                            DemandExists := DemandInvtProfile.Next <> 0;

                                    NS_OnBeforePlanStepSettingOnStartOver(SupplyInvtProfile, DemandInvtProfile, SupplyExists, DemandExists);
                            ...
                            NextState::MatchDates:
                                begin
                                    ...
                                    if DemandInvtProfile."Due Date" < SupplyInvtProfile."Due Date" then begin
                                        if NS_CheckScheduleIn(SupplyInvtProfile, DemandInvtProfile."Due Date", NewSupplyDate, true) then
                                            WeAreSureThatDatesMatch := true
                                        else
                                            NextState := NextState::CreateSupply;
                                    end else
                                        if DemandInvtProfile."Due Date" > SupplyInvtProfile."Due Date" then begin
                                            if NS_CheckScheduleOut(SupplyInvtProfile, DemandInvtProfile."Due Date", NewSupplyDate, true) then
                                                WeAreSureThatDatesMatch := not NS_ScheduleAllOutChangesSequence(SupplyInvtProfile, NewSupplyDate)
                                            else
                                                NextState := NextState::ReduceSupply;
                                        end else
                                            WeAreSureThatDatesMatch := true;
                                    ...
                                    if WeAreSureThatDatesMatch and IsReorderPointPlanning then begin
                                        ...
                                        if ROPHasBeenCrossed then begin
                                            NS_CreateSupplyForward(SupplyInvtProfile, LatestBucketStartDate,
                                    ...
                                    if WeAreSureThatDatesMatch then begin
                                        if NewSupplyDate <> OriginalSupplyDate then
                                            NS_Reschedule(SupplyInvtProfile, NewSupplyDate, 0T);
                            ...
                            NextState::CreateSupply:
                                begin
                                    ...
                                    if NS_FromLotAccumulationPeriodStartDate(LotAccumulationPeriodStartDate, DemandInvtProfile."Due Date") then
                                    ...
                                    if (NewSupplyDate >= LatestBucketStartDate) and IsReorderPointPlanning then
                                        NS_MaintainProjectedInv(
                                          InvChangeReminder, NewSupplyDate, LastProjectedInventory, LatestBucketStartDate, ROPHasBeenCrossed);
                                    if ROPHasBeenCrossed then begin
                                        NS_CreateSupplyForward(SupplyInvtProfile, LatestBucketStartDate, LastProjectedInventory,
                                    ...
                                    if WeAreSureThatDatesMatch then begin
                                        ...
                                        NS_CreateSupply(SupplyInvtProfile, DemandInvtProfile,
                                          LastProjectedInventory +
                                          NS_QtyFromPendingReminders(InvChangeReminder, DemandInvtProfile."Due Date", LatestBucketStartDate) -
                            ...
                            NextState::ReduceSupply:
                                begin
                                    ...
                                    if not NewSupplyHasTakenOver then
                                        if NS_DecreaseQty(SupplyInvtProfile, SupplyInvtProfile."Untracked Quantity") then
                                            NextState := NextState::CloseSupply
                                        else begin
                            ...
                            NextState::CloseDemand:
                                begin
                                    ...
                                    if DemandInvtProfile."Order Relation" = DemandInvtProfile."Order Relation"::"Safety Stock" then begin
                                        NS_AllocateSafetystock(SupplyInvtProfile, DemandInvtProfile."Untracked Quantity", DemandInvtProfile."Due Date");
                                        if IsReorderPointPlanning and (SupplyInvtProfile."Due Date" >= LatestBucketStartDate) then
                                            NS_PostInvChgReminder(InvChangeReminder, SupplyInvtProfile, true);
                                    end else begin
                                        if IsReorderPointPlanning then
                                            NS_PostInvChgReminder(InvChangeReminder, DemandInvtProfile, false);
                                        ...
                                        if DemandInvtProfile."Untracked Quantity" <> 0 then begin
                                            ...
                                            if SupplyInvtProfile."Action Message" <> SupplyInvtProfile."Action Message"::" " then
                                                NS_MaintainPlanningLine(SupplyInvtProfile, PlanningLineStage::"Line Created", ScheduleDirection::Backward);
                                            SupplyInvtProfile.Modify;

                                            if IsReorderPointPlanning and (SupplyInvtProfile."Due Date" >= LatestBucketStartDate) then
                                                NS_PostInvChgReminder(InvChangeReminder, SupplyInvtProfile, true);

                                            NS_CheckSupplyAndTrack(DemandInvtProfile, SupplyInvtProfile);
                            ...
                            NextState::CloseSupply:
                                begin
                                    ...
                                    then begin
                                        NS_AllocateSafetystock(SupplyInvtProfile, SupplyInvtProfile."Untracked Quantity", DemandInvtProfile."Due Date");
                                        if IsReorderPointPlanning and (SupplyInvtProfile."Due Date" >= LatestBucketStartDate) then
                                            NS_PostInvChgReminder(InvChangeReminder, SupplyInvtProfile, true);
                                    end else begin
                                        if IsReorderPointPlanning and (SupplyInvtProfile."Due Date" >= LatestBucketStartDate) then
                                            NS_PostInvChgReminder(InvChangeReminder, SupplyInvtProfile, false);
                                        ...
                                        then
                                            if DemandExists then
                                                NS_Track(SupplyInvtProfile, DemandInvtProfile, false, false, SupplyInvtProfile.Binding.AsInteger())
                                            else
                                                NS_Track(SupplyInvtProfile, DemandInvtProfile, true, false, SupplyInvtProfile.Binding::" ".AsInteger());
                                    ...
                                    if TempSKU."Maximum Order Quantity" > 0 then
                                        NS_CheckSupplyRemQtyAndUntrackQty(SupplyInvtProfile);
                                    SupplyExists := SupplyInvtProfile.Next <> 0;
                                    NextState := NextState::StartOver;
                                end;
                            NextState::CloseLoop:
        ...
        NS_SetAcceptAction(TempSKU."Item No.");
    end;

    local procedure NS_PlanItemCalcInitialInventory(var DemandInvtProfile: Record "Inventory Profile"; var SupplyInvtProfile: Record "Inventory Profile"; PlanningStartDate: Date; var DemandExists: Boolean; var SupplyExists: Boolean; var LastProjectedInventory: Decimal)
    begin
        DemandInvtProfile.SetRange("Due Date", 0D, PlanningStartDate - 1);
        SupplyInvtProfile.SetRange("Due Date", 0D, PlanningStartDate - 1);
        DemandExists := DemandInvtProfile.FindSet;
        SupplyExists := SupplyInvtProfile.FindSet;
        NS_OnBeforePrePlanDateApplicationLoop(SupplyInvtProfile, DemandInvtProfile, SupplyExists, DemandExists);
        while DemandExists and SupplyExists do begin
            NS_OnStartOfPrePlanDateApplicationLoop(SupplyInvtProfile, DemandInvtProfile, SupplyExists, DemandExists);
            if DemandInvtProfile."Untracked Quantity" > SupplyInvtProfile."Untracked Quantity" then begin
                LastProjectedInventory += SupplyInvtProfile."Remaining Quantity (Base)";
                DemandInvtProfile."Untracked Quantity" -= SupplyInvtProfile."Untracked Quantity";
                NS_FrozenZoneTrack(SupplyInvtProfile, DemandInvtProfile);
                SupplyInvtProfile."Untracked Quantity" := 0;
                SupplyInvtProfile.Modify;
                SupplyExists := SupplyInvtProfile.Next <> 0;
            end else begin
                LastProjectedInventory -= DemandInvtProfile."Remaining Quantity (Base)";
                SupplyInvtProfile."Untracked Quantity" -= DemandInvtProfile."Untracked Quantity";
                NS_FrozenZoneTrack(DemandInvtProfile, SupplyInvtProfile);
                DemandInvtProfile."Untracked Quantity" := 0;
                DemandInvtProfile.Modify;
                DemandExists := DemandInvtProfile.Next <> 0;
            ...
            NS_OnEndOfPrePlanDateApplicationLoop(SupplyInvtProfile, DemandInvtProfile, SupplyExists, DemandExists);
        end;
    end;

    local procedure NS_PlanItemNextStateMatchQty(var DemandInventoryProfile: Record "Inventory Profile"; var SupplyInventoryProfile: Record "Inventory Profile"; var LastProjectedInventory: Decimal; IsReorderPointPlanning: Boolean; RespectPlanningParm: Boolean)
    begin
        case true of
            SupplyInventoryProfile."Untracked Quantity" >= DemandInventoryProfile."Untracked Quantity":
                NextState := NextState::CloseDemand;
            NS_ShallSupplyBeClosed(SupplyInventoryProfile, DemandInventoryProfile."Due Date", IsReorderPointPlanning):
                NextState := NextState::CloseSupply;
            NS_IncreaseQtyToMeetDemand(
              SupplyInventoryProfile, DemandInventoryProfile, true, RespectPlanningParm,
              not NS_SKURequiresLotAccumulation(TempSKU)):
                begin
                    NextState := NextState::CloseDemand;
    ...
    local procedure NS_FilterDemandSupplyRelatedToSKU(var InventoryProfile: Record "Inventory Profile")
    begin
    ...
    local procedure NS_ScheduleForward(var SupplyInvtProfile: Record "Inventory Profile"; StartingDate: Date)
    begin
        SupplyInvtProfile."Starting Date" := StartingDate;
        NS_MaintainPlanningLine(SupplyInvtProfile, PlanningLineStage::"Routing Created", ScheduleDirection::Forward);
    ...
    local procedure NS_IncreaseQtyToMeetDemand(var SupplyInvtProfile: Record "Inventory Profile"; DemandInvtProfile: Record "Inventory Profile"; LimitedHorizon: Boolean; RespectPlanningParm: Boolean; CheckSourceType: Boolean): Boolean
    var
    ...
    begin
        ...
        if LimitedHorizon then
            if not NS_AllowLotAccumulation(SupplyInvtProfile, DemandInvtProfile."Due Date") then
                exit(false);
    ...
    local procedure NS_IncreaseQty(var SupplyInvtProfile: Record "Inventory Profile"; NeededQty: Decimal; RespectPlanningParm: Boolean)
    var
        TempQty: Decimal;
    begin
        ...
        then begin
            ...
            //else
            // SupplyInvtProfile."Action Message" := SupplyInvtProfile."Action Message"::"Resched.& Chg. Qty."; //PPNA16.0 Blocked
    ...
    local procedure NS_DecreaseQty(var SupplyInvtProfile: Record "Inventory Profile"; ReduceQty: Decimal): Boolean
    var
        TempQty: Decimal;
    begin
        if not NS_CanDecreaseSupply(SupplyInvtProfile, ReduceQty) then begin
    ...
    local procedure NS_CanDecreaseSupply(InventoryProfileSupply: Record "Inventory Profile"; var ReduceQty: Decimal): Boolean
    var
        TrackedQty: Decimal;
    begin
    ...
    local procedure NS_CreateSupply(var SupplyInvtProfile: Record "Inventory Profile"; var DemandInvtProfile: Record "Inventory Profile"; ProjectedInventory: Decimal; IsExceptionOrder: Boolean; RespectPlanningParm: Boolean)
    var
        ReorderQty: Decimal;
    begin
        NS_InitSupply(SupplyInvtProfile, 0, DemandInvtProfile."Due Date");
        ...
        if (not IsExceptionOrder) or RespectPlanningParm then begin
            if not RespectPlanningParm then
                ReorderQty := NS_CalcReorderQty(ReorderQty, ProjectedInventory, SupplyInvtProfile."Line No.")
            else
                if IsExceptionOrder then begin
            ...
            ReorderQty += NS_AdjustReorderQty(ReorderQty, TempSKU, SupplyInvtProfile."Line No.", SupplyInvtProfile."Min. Quantity");
            SupplyInvtProfile."Max. Quantity" := TempSKU."Maximum Order Quantity";
        end;
        NS_UpdateQty(SupplyInvtProfile, ReorderQty);
        ...
        if (not IsExceptionOrder or RespectPlanningParm) and (OverflowLevel > 0) then
            ...
            NS_CheckNewOverflow(SupplyInvtProfile, ProjectedInventory + ReorderQty, ReorderQty, SupplyInvtProfile."Due Date");
    end;

    local procedure NS_CreateDemand(var DemandInvtProfile: Record "Inventory Profile"; var SKU: Record "Stockkeeping Unit"; NeededQuantity: Decimal; NeededDueDate: Date; OrderRelation: Option Normal,"Safety Stock","Reorder Point")
    begin
        DemandInvtProfile.Init;
        DemandInvtProfile."Line No." := NS_NextLineNo;
        ...
        NS_OnBeforeDemandInvtProfileInsert(DemandInvtProfile, SKU);
        DemandInvtProfile.Insert;
    end;

    local procedure NS_Track(FromProfile: Record "Inventory Profile"; ToProfile: Record "Inventory Profile"; IsSurplus: Boolean; IssueActionMessage: Boolean; Binding: Option " ","Order-to-Order")
    var
    ...
    begin
        ...
        DecreaseSupply :=
          ...
          (FromProfile."Action Message" in [FromProfile."Action Message"::"Change Qty."]) and
        ...
        then begin
            ...
            if FromProfile.IsSupply then begin
                ...
                case FromProfile."Action Message" of
                    //FromProfile."Action Message"::"Resched.& Chg. Qty.", //PPNA16.0 Blocked
                      ...
                      FromProfile."Action Message"::"Change Qty.":
                        begin
                            if (SplitQty > 0) and (SplitQty < TrackQty) then begin
                                ...
                                //PPNA16.0 Added Start
                                FromProfile.TransferToTrackingEntry(TrkgReservEntryArray[1],
                                   (FromProfile."Action Message" = FromProfile."Action Message"::Reschedule));
                                //PPNA16.0 Added End
                ...
                if SplitQty2 > 0 then begin
                    ...
                    NS_PrepareTempTracking(TrkgReservEntryArray[5], TrkgReservEntryArray[6], IsSurplus, IssueActionMessage, Binding);
                end;
        ...
        case SplitState of
            SplitState::NoSplit:
                NS_PrepareTempTracking(TrkgReservEntryArray[1], TrkgReservEntryArray[2], IsSurplus, IssueActionMessage, Binding);
            SplitState::SplitFromProfile:
                if IsSurplus then begin
                    NS_PrepareTempTracking(TrkgReservEntryArray[1], TrkgReservEntryArray[4], false, IssueActionMessage, Binding);
                    NS_PrepareTempTracking(TrkgReservEntryArray[3], TrkgReservEntryArray[4], true, IssueActionMessage, Binding);
                end else begin
                    TrkgReservEntryArray[4] := TrkgReservEntryArray[2];
                    NS_PrepareTempTracking(TrkgReservEntryArray[1], TrkgReservEntryArray[2], IsSurplus, IssueActionMessage, Binding);
                    NS_PrepareTempTracking(TrkgReservEntryArray[3], TrkgReservEntryArray[4], IsSurplus, IssueActionMessage, Binding);
                end;
            SplitState::SplitToProfile:
                begin
                    TrkgReservEntryArray[4] := TrkgReservEntryArray[1];
                    NS_PrepareTempTracking(TrkgReservEntryArray[2], TrkgReservEntryArray[1], IsSurplus, IssueActionMessage, Binding);
                    NS_PrepareTempTracking(TrkgReservEntryArray[3], TrkgReservEntryArray[4], IsSurplus, IssueActionMessage, Binding);
                end;
            SplitState::Cancel:
                NS_PrepareTempTracking(TrkgReservEntryArray[1], TrkgReservEntryArray[3], IsSurplus, IssueActionMessage, Binding);
        end;
    end;

    local procedure NS_PrepareTempTracking(var FromTrkgReservEntry: Record "Reservation Entry"; var ToTrkgReservEntry: Record "Reservation Entry"; IsSurplus: Boolean; IssueActionMessage: Boolean; Binding: Option " ","Order-to-Order")
    begin
        ...
        if IsSurplus or (ToTrkgReservEntry."Reservation Status" = ToTrkgReservEntry."Reservation Status"::Surplus) then begin
            ...
            NS_InsertTempTracking(FromTrkgReservEntry, ToTrkgReservEntry);
            exit;
        end;
        ...
        if FromTrkgReservEntry."Reservation Status" = FromTrkgReservEntry."Reservation Status"::Surplus then begin
            ...
            NS_InsertTempTracking(ToTrkgReservEntry, FromTrkgReservEntry);
            exit;
        end;
    ...
    local procedure NS_InsertTempTracking(var FromTrkgReservEntry: Record "Reservation Entry"; var ToTrkgReservEntry: Record "Reservation Entry")
    var
        NextEntryNo: Integer;
    ...
    begin
        ...
        if FromTrkgReservEntry."Reservation Status" = FromTrkgReservEntry."Reservation Status"::Surplus then begin
            ...
            NS_SetQtyToHandle(TempTrkgReservEntry);
            TempTrkgReservEntry.Insert;
        end else begin
            NS_MatchReservationEntries(FromTrkgReservEntry, ToTrkgReservEntry);
            ...
            if FromTrkgReservEntry.Positive then
                ShouldInsert := NS_ShouldInsertTrackingEntry(FromTrkgReservEntry)
            else
                ShouldInsert := NS_ShouldInsertTrackingEntry(ToTrkgReservEntry);
    ...
    local procedure NS_SetQtyToHandle(var TrkgReservEntry: Record "Reservation Entry")
    var
        PickedQty: Decimal;
    begin
        ...
        PickedQty := NS_QtyPickedForSourceDocument(TrkgReservEntry);
    ...
    local procedure NS_CommitTracking()
    var
    ...
    begin
        ...
        repeat
            ...
            NS_UpdateAppliedItemEntry(ReservEntry);
            ReservEntry.Insert;
    ...
    local procedure NS_MaintainPlanningLine(var SupplyInvtProfile: Record "Inventory Profile"; NewPhase: Option " ","Line Created","Routing Created",Exploded,Obsolete; Direction: Option Forward,Backward)
    var
    ...
    begin
                    ...
                    then begin
                        ...
                        case SupplyInvtProfile."Source Type" of
                            DATABASE::"Purchase Line":
                                NS_SetPurchase(PurchaseLine, SupplyInvtProfile);
                            DATABASE::"Prod. Order Line":
                                NS_SetProdOrder(ProdOrderLine, SupplyInvtProfile);
                            DATABASE::"Assembly Header":
                                NS_SetAssembly(AsmHeader, SupplyInvtProfile);
                            DATABASE::"Transfer Line":
                                NS_SetTransfer(TransLine, SupplyInvtProfile);
                        end;
                    NS_AdjustPlanLine(SupplyInvtProfile);
        ...
        if NewPhase = NewPhase::Exploded then begin
            ...
            then begin
                NS_AdjustTransferDates(ReqLine);
                if ReqLine."Action Message" = ReqLine."Action Message"::New then begin
                    CrntSupplyInvtProfile.Copy(SupplyInvtProfile);
                    SupplyInvtProfile.Init;
                    SupplyInvtProfile."Line No." := NS_NextLineNo;
                    ...
                    NS_SynchronizeTransferProfiles(SupplyInvtProfile, ReqLine);
            end;
        end;
    ...
    [Scope('Cloud')]
    procedure NS_AdjustReorderQty(OrderQty: Decimal; SKU: Record "Stockkeeping Unit"; SupplyLineNo: Integer; MinQty: Decimal): Decimal
    var
        DeltaQty: Decimal;
        Rounding: Decimal;
    begin
    ...
    local procedure NS_CalcInventoryProfileRemainingQty(var InventoryProfile: Record "Inventory Profile"; DocumentNo: Code[20]): Decimal
    begin
    ...
    local procedure NS_CalcReorderQty(NeededQty: Decimal; ProjectedInventory: Decimal; SupplyLineNo: Integer) QtyToOrder: Decimal
    var
        Item: Record Item;
        SKU: Record "Stockkeeping Unit";
    begin
    ...
    local procedure NS_CalcOrderQty(NeededQty: Decimal; ProjectedInventory: Decimal; SupplyLineNo: Integer) QtyToOrder: Decimal
    begin
        QtyToOrder := NS_CalcReorderQty(NeededQty, ProjectedInventory, SupplyLineNo);
    ...
    local procedure NS_CalcSalesOrderQty(AsmLine: Record "Assembly Line") QtyOnSalesOrder: Decimal
    var
    ...
    local procedure NS_AdjustPlanLine(var Supply: Record "Inventory Profile")
    begin
        with Supply do begin
            ...
            if "Action Message" in
               ["Action Message"::New,
                ...
                //"Action Message"::"Resched.& Chg. Qty.", //PPNA16.0 Blocked
    ...
    local procedure NS_DisableRelations()
    var
        PlanningComponent: Record "Planning Component";
    ...
    local procedure NS_SynchronizeTransferProfiles(var InventoryProfile: Record "Inventory Profile"; var TransferReqLine: Record "Requisition Line")
    var
    ...
    begin
        ...
        if NS_GetTransferSisterProfile(SupplyInvtProfile, InventoryProfile) then begin
            NS_TransferReqLineToInvProfiles(InventoryProfile, TransferReqLine);
            InventoryProfile.Modify;
        end;
        InventoryProfile.Copy(SupplyInvtProfile);
    end;

    local procedure NS_TransferReqLineToInvProfiles(var InventoryProfile: Record "Inventory Profile"; var TransferReqLine: Record "Requisition Line")
    begin
        with InventoryProfile do begin
    ...
    local procedure NS_SyncTransferDemandWithReqLine(var InventoryProfile: Record "Inventory Profile"; LocationCode: Code[10])
    var
        TransferReqLine: Record "Requisition Line";
    begin
    ...
    local procedure NS_GetTransferSisterProfile(CurrInvProfile: Record "Inventory Profile"; var SisterInvProfile: Record "Inventory Profile") Ok: Boolean
    begin
    ...
    local procedure NS_AdjustTransferDates(var TransferReqLine: Record "Requisition Line")
    var
    ...
    local procedure NS_InsertTempTransferSKU(var TransLine: Record "Transfer Line")
    var
        SKU: Record "Stockkeeping Unit";
    begin
    ...
    local procedure NS_UpdateTempSKUTransferLevels()
    var
        SKU: Record "Stockkeeping Unit";
    begin
        SKU.Copy(TempSKU);
    ...
    local procedure NS_CancelTransfer(var SupplyInvtProfile: Record "Inventory Profile"; var DemandInvtProfile: Record "Inventory Profile"; DemandExists: Boolean) Cancel: Boolean
    var
    ...
    begin
        ...
        if NS_GetTransferSisterProfile(DemandInvtProfile, SupplyInvtProfile) then begin
            if SupplyInvtProfile."Action Message" = SupplyInvtProfile."Action Message"::New then
                SupplyInvtProfile.FieldError("Action Message");
    ...
    local procedure NS_PostInvChgReminder(var InvChangeReminder: Record "Inventory Profile"; InvProfile: Record "Inventory Profile"; PostOnlyMinimum: Boolean)
    begin
        ...
        NS_OnBeforePostInvChgReminder(InvChangeReminder, InvProfile, PostOnlyMinimum);
        ...
        NS_OnAfterPostInvChgReminder(InvChangeReminder, InvProfile, PostOnlyMinimum);
    end;

    local procedure NS_QtyFromPendingReminders(var InvChangeReminder: Record "Inventory Profile"; AtDate: Date; LatestBucketStartDate: Date) PendingQty: Decimal
    var
    ...
    local procedure NS_MaintainProjectedInv(var InvChangeReminder: Record "Inventory Profile"; AtDate: Date; var LastProjectedInventory: Decimal; var LatestBucketStartDate: Date; var ROPHasBeenCrossed: Boolean)
    var
    ...
    begin
        ...
        LatestBucketStartDate := NS_FindNextBucketStartDate(InvChangeReminder, AtDate, LatestBucketStartDate);
    ...
    local procedure NS_FindNextBucketStartDate(var InvChangeReminder: Record "Inventory Profile"; AtDate: Date; LatestBucketStartDate: Date) NextBucketStartDate: Date
    var
    ...
    local procedure NS_SetIgnoreOverflow(var SupplyInvtProfile: Record "Inventory Profile")
    begin
    ...
    local procedure NS_ChkInitialOverflow(var DemandInvtProfile: Record "Inventory Profile"; var SupplyInvtProfile: Record "Inventory Profile"; OverflowLevel: Decimal; InventoryLevel: Decimal; FromDate: Date; ToDate: Date)
    var
        xDemandInvtProfile: Record "Inventory Profile";
        xSupplyInvtProfile: Record "Inventory Profile";
        OverflowQty: Decimal;
    ...
    begin
        ...
        if OverflowLevel > 0 then begin
                    ...
                    if SupplyInvtProfile.Find('-') then begin
                        ...
                        repeat
                            if OverflowQty > 0 then begin
                                ...
                                NS_DecreaseQty(SupplyInvtProfile, OverflowQty);
                                ...
                                then
                                    if NS_CanDecreaseSupply(SupplyInvtProfile, OverflowQty) then
                                        NS_DecreaseQty(SupplyInvtProfile, DecreasedSupplyQty);
            ...
            if OverflowLevel = 0 then
                NS_SetIgnoreOverflow(SupplyInvtProfile);

        DemandInvtProfile.Copy(xDemandInvtProfile);
        SupplyInvtProfile.Copy(xSupplyInvtProfile);
    end;

    local procedure NS_CheckNewOverflow(var SupplyInvtProfile: Record "Inventory Profile"; InventoryLevel: Decimal; QtyToDecreaseOverFlow: Decimal; LastDueDate: Date)
    var
        xSupplyInvtProfile: Record "Inventory Profile";
        OriginalSupplyQty: Decimal;
        ...
        if SupplyInvtProfile.FindSet(true) then
            repeat
                ...
                if InventoryLevel > OverflowLevel then begin
                    ...
                    NS_DecreaseQty(SupplyInvtProfile, QtyToDecrease);
    ...
    local procedure NS_CheckScheduleIn(var SupplyInvtProfile: Record "Inventory Profile"; TargetDate: Date; var PossibleDate: Date; LimitedHorizon: Boolean): Boolean
    begin
        if SupplyInvtProfile."Planning Flexibility" <> SupplyInvtProfile."Planning Flexibility"::Unlimited then
            exit(false);

        if LimitedHorizon and not NS_AllowScheduleIn(SupplyInvtProfile, TargetDate) then
    ...
    local procedure NS_CheckScheduleOut(var SupplyInvtProfile: Record "Inventory Profile"; TargetDate: Date; var PossibleDate: Date; LimitedHorizon: Boolean): Boolean
    begin
        NS_OnBeforeCheckScheduleOut(SupplyInvtProfile, TempSKU, BucketSize);
        ...
        if (TargetDate - SupplyInvtProfile."Due Date") <= DampenersDays then
            ...
            then
                ...
                if NS_AllowScheduleOut(SupplyInvtProfile, TargetDate) then
                    PossibleDate := TargetDate
                else begin
                    // Do not reschedule but may be lot accumulation is still an option
                    PossibleDate := SupplyInvtProfile."Due Date";
                    if SupplyInvtProfile."Fixed Date" <> 0D then
                        exit(NS_AllowLotAccumulation(SupplyInvtProfile, TargetDate));
                    exit(false);
                end;
        ...
        then begin
            if not NS_AllowLotAccumulation(SupplyInvtProfile, TargetDate) then // but reschedule only if lot accumulation is allowed for target date
                exit(false);
    ...
    local procedure NS_CheckSupplyWithSKU(var InventoryProfile: Record "Inventory Profile"; var SKU: Record "Stockkeeping Unit")
    var
    ...
    local procedure NS_CreateSupplyForward(var SupplyInvtProfile: Record "Inventory Profile"; AtDate: Date; ProjectedInventory: Decimal; var NewSupplyHasTakenOver: Boolean; CurrDueDate: Date)
    var
    ...
    begin
        ...
        NS_InitSupply(TempSupplyInvtProfile, 0, AtDate);

        // Make sure VAR boolean is reset:
        NewSupplyHasTakenOver := false;
        QtyToOrder := NS_CalcOrderQty(QtyToOrder, ProjectedInventory, TempSupplyInvtProfile."Line No.");

        // Use new supplyprofile to determine lead-time
        NS_UpdateQty(TempSupplyInvtProfile, QtyToOrder + NS_AdjustReorderQty(QtyToOrder, TempSKU, TempSupplyInvtProfile."Line No.", 0));
        TempSupplyInvtProfile.Insert;
        NS_ScheduleForward(TempSupplyInvtProfile, AtDate);
        LeadTimeEndDate := TempSupplyInvtProfile."Due Date";

        // Find supply within leadtime, returns a qty
        SupplyWithinLeadtime := NS_SumUpProjectedSupply(SupplyInvtProfile, AtDate, LeadTimeEndDate);
        ...
        if SupplyWithinLeadtime + ProjectedInventory > TempSKU."Reorder Point" then begin
            ...
            NS_MaintainPlanningLine(TempSupplyInvtProfile, PlanningLineStage::Obsolete, ScheduleDirection::Backward);
        ...
        while QtyToOrder > 0 do begin
            ...
            if HasLooped then begin
                ...
                QtyToOrderThisLine := QtyToOrder + NS_AdjustReorderQty(QtyToOrder, TempSKU, TempSupplyInvtProfile."Line No.", 0);
                NS_UpdateQty(TempSupplyInvtProfile, QtyToOrderThisLine);
                TempSupplyInvtProfile.Insert;
                NS_ScheduleForward(TempSupplyInvtProfile, AtDate);
            end else begin
                QtyToOrderThisLine := QtyToOrder + NS_AdjustReorderQty(QtyToOrder, TempSKU, TempSupplyInvtProfile."Line No.", 0);
        ...
        if HasLooped and (OverflowLevel > 0) then
            // the new supply might cause overflow in inventory since
            // it wasn't considered when Overflow was calculated
            NS_CheckNewOverflow(SupplyInvtProfile, ProjectedInventory + QtyToDecreaseOverFlow, QtyToDecreaseOverFlow, LeadTimeEndDate);

        SupplyInvtProfile := CurrSupplyInvtProfile;
    end;

    local procedure NS_AllowScheduleIn(SupplyInvtProfile: Record "Inventory Profile"; TargetDate: Date) CanReschedule: Boolean
    begin
        CanReschedule := CalcDate(TempSKU."Rescheduling Period", TargetDate) >= SupplyInvtProfile."Due Date";
    end;

    local procedure NS_AllowScheduleOut(SupplyInvtProfile: Record "Inventory Profile"; TargetDate: Date) CanReschedule: Boolean
    begin
        CanReschedule := CalcDate(TempSKU."Rescheduling Period", SupplyInvtProfile."Due Date") >= TargetDate;
    end;

    local procedure NS_AllowLotAccumulation(SupplyInvtProfile: Record "Inventory Profile"; DemandDueDate: Date) AccumulationOK: Boolean
    begin
        AccumulationOK := CalcDate(TempSKU."Lot Accumulation Period", SupplyInvtProfile."Due Date") >= DemandDueDate;
    end;

    local procedure NS_ShallSupplyBeClosed(SupplyInventoryProfile: Record "Inventory Profile"; DemandDueDate: Date; IsReorderPointPlanning: Boolean): Boolean
    var
        CloseSupply: Boolean;
    begin
        if SupplyInventoryProfile."Is Exception Order" then begin
            if TempSKU."Reordering Policy" = TempSKU."Reordering Policy"::"Lot-for-Lot" then
                ...
                CloseSupply := not NS_AllowLotAccumulation(SupplyInventoryProfile, DemandDueDate)
            else
    ...
    local procedure NS_NextLineNo(): Integer
    begin
        LineNo += 1;
        exit(LineNo);
    end;

    local procedure NS_Reschedule(var SupplyInvtProfile: Record "Inventory Profile"; TargetDate: Date; TargetTime: Time)
    begin
        SupplyInvtProfile.TestField("Planning Flexibility", SupplyInvtProfile."Planning Flexibility"::Unlimited.AsInteger());
    ...
    local procedure NS_InitSupply(var SupplyInvtProfile: Record "Inventory Profile"; OrderQty: Decimal; DueDate: Date)
    var
        Item: Record Item;
    ...
    begin
        ...
        SupplyInvtProfile."Action Message" := SupplyInvtProfile."Action Message"::New;
        NS_UpdateQty(SupplyInvtProfile, OrderQty);
    ...
    local procedure NS_UpdateQty(var InvProfile: Record "Inventory Profile"; Qty: Decimal)
    begin
    ...
    local procedure NS_TransferAttributes(var ToInvProfile: Record "Inventory Profile"; var FromInvProfile: Record "Inventory Profile")
    begin
    ...
    local procedure NS_AllocateSafetystock(var SupplyInvtProfile: Record "Inventory Profile"; QtyToAllocate: Decimal; AtDate: Date)
    var
    ...
    local procedure NS_SumUpProjectedSupply(var SupplyInvtProfile: Record "Inventory Profile"; FromDate: Date; ToDate: Date) ProjectedQty: Decimal
    var
        xSupplyInvtProfile: Record "Inventory Profile";
    begin
    ...
    local procedure NS_SumUpAvailableSupply(var SupplyInvtProfile: Record "Inventory Profile"; FromDate: Date; ToDate: Date) AvailableQty: Decimal
    var
        xSupplyInvtProfile: Record "Inventory Profile";
    begin
    ...
    local procedure NS_SetPriority(var InvProfile: Record "Inventory Profile"; IsReorderPointPlanning: Boolean; ToDate: Date)
    begin
        with InvProfile do begin
            ...
            NS_OnAfterSetOrderPriority(InvProfile);
            ...
            NS_SetAttributePriority(InvProfile);

            Modify;
        end;
    end;

    local procedure NS_SetAttributePriority(var InvProfile: Record "Inventory Profile")
    var
        HandleLot: Boolean;
        HandleSN: Boolean;
    begin
    ...
    local procedure NS_UpdatePriorities(var InvProfile: Record "Inventory Profile"; IsReorderPointPlanning: Boolean; ToDate: Date)
    var
    ...
    begin
        ...
        if InvProfile.FindSet(true) then
            repeat
                NS_SetPriority(InvProfile, IsReorderPointPlanning, ToDate);
            until InvProfile.Next = 0;
        InvProfile.Copy(xInvProfile);
    end;

    local procedure NS_InsertSafetyStockDemands(var DemandInvtProfile: Record "Inventory Profile"; PlanningStartDate: Date)
    var
        xDemandInvtProfile: Record "Inventory Profile";
    ...
    begin
        ...
        NS_OnAfterInsertSafetyStockDemands(
          DemandInvtProfile, xDemandInvtProfile, TempSafetyStockInvtProfile, TempSKU, PlanningStartDate, PlanToDate);
    end;

    local procedure NS_ScheduleAllOutChangesSequence(var SupplyInvtProfile: Record "Inventory Profile"; NewDate: Date): Boolean
    var
        xSupplyInvtProfile: Record "Inventory Profile";
    ...
    begin
        ...
        if not NS_AllowScheduleOut(SupplyInvtProfile, NewDate) then
            exit(false);
        ...
        while Continue do begin
            ...
            if TryRescheduleSupply then begin
                NS_Reschedule(TempRescheduledSupplyInvtProfile, NewDate, 0T);
    ...
    local procedure NS_PrepareOrderToOrderLink(var InventoryProfile: Record "Inventory Profile")
    begin
    ...
    local procedure NS_SetAcceptAction(ItemNo: Code[20])
    var
        ReqLine: Record "Requisition Line";
    ...
    [Scope('Personalization')]
    procedure NS_GetRouting(var ReqLine: Record "Requisition Line")
    var
    ...
    [Scope('Personalization')]
    procedure NS_GetComponents(var ReqLine: Record "Requisition Line")
    var
    ...
    [Scope('Personalization')]
    procedure NS_Recalculate(var ReqLine: Record "Requisition Line"; Direction: Option Forward,Backward; RefreshRouting: Boolean)
    begin
        with ReqLine do begin
            PlngLnMgt.Calculate(
    ...
    [Scope('Personalization')]
    procedure NS_GetPlanningCompList(var PlanningCompList: Record "Planning Component" temporary)
    begin
    ...
    [Scope('Personalization')]
    procedure NS_SetParm(Forecast: Code[10]; ExclBefore: Date; WorksheetType: Option Requisition,Planning; pJobNoFilter: Code[20])
    begin
        //ProjectPro. Added param: ;pJobNoFilter@1100773000 : Code[20]
        //SPLN: pJobNoFilter='' in all calls
    ...
    [Scope('Personalization')]
    procedure NS_SetResiliencyOn()
    begin
        PlanningResilicency := true;
    end;

    [Scope('Personalization')]
    procedure NS_GetResiliencyError(var PlanningErrorLog: Record "Planning Error Log"): Boolean
    begin
    ...
    local procedure NS_CloseTracking(ReservEntry: Record "Reservation Entry"; var SupplyInventoryProfile: Record "Inventory Profile"; ToDate: Date): Boolean
    var
    ...
    local procedure NS_FrozenZoneTrack(FromInventoryProfile: Record "Inventory Profile"; ToInventoryProfile: Record "Inventory Profile")
    begin
        if FromInventoryProfile.TrackingExists then
            NS_Track(FromInventoryProfile, ToInventoryProfile, true, false, FromInventoryProfile.Binding::" ".AsInteger());
        ...
        if ToInventoryProfile.TrackingExists then begin
            ...
            NS_Track(ToInventoryProfile, FromInventoryProfile, true, false, ToInventoryProfile.Binding::" ".AsInteger());
        end;
    end;

    local procedure NS_ExceedROPinException(RespectPlanningParm: Boolean): Boolean
    begin
        if not RespectPlanningParm then
            exit(false);
    ...
    local procedure NS_CreateSupplyForInitialSafetyStockWarning(var SupplyInventoryProfile: Record "Inventory Profile"; ProjectedInventory: Decimal; var LastProjectedInventory: Decimal; var LastAvailableInventory: Decimal; PlanningStartDate: Date; RespectPlanningParm: Boolean; IsReorderPointPlanning: Boolean)
    var
        OrderQty: Decimal;
        ReorderQty: Decimal;
    begin
        OrderQty := TempSKU."Safety Stock Quantity" - ProjectedInventory;
        if NS_ExceedROPinException(RespectPlanningParm) then
            OrderQty := TempSKU."Reorder Point" - ProjectedInventory;

        ReorderQty := OrderQty;

        repeat
            NS_InitSupply(SupplyInventoryProfile, ReorderQty, PlanningStartDate);
            if RespectPlanningParm then begin
                if IsReorderPointPlanning then
                    ReorderQty := NS_CalcOrderQty(ReorderQty, ProjectedInventory, SupplyInventoryProfile."Line No.");

                ReorderQty += NS_AdjustReorderQty(ReorderQty, TempSKU, SupplyInventoryProfile."Line No.", SupplyInventoryProfile."Min. Quantity");
                SupplyInventoryProfile."Max. Quantity" := TempSKU."Maximum Order Quantity";
                NS_UpdateQty(SupplyInventoryProfile, ReorderQty);
            ...
            OrderQty -= ReorderQty;
            if NS_ExceedROPinException(RespectPlanningParm) and (OrderQty = 0) then
                OrderQty := ExceedROPqty;
            ReorderQty := OrderQty;
        until OrderQty <= 0; // Create supplies until Safety Stock is met or Reorder point is exceeded
    end;

    local procedure NS_IsTrkgForSpecialOrderOrDropShpt(ReservEntry: Record "Reservation Entry"): Boolean
    var
        SalesLine: Record "Sales Line";
        PurchLine: Record "Purchase Line";
    begin
    ...
    local procedure NS_CheckSupplyRemQtyAndUntrackQty(var InventoryProfile: Record "Inventory Profile")
    var
        RemQty: Decimal;
    begin
        with InventoryProfile do begin
    ...
    local procedure NS_CheckItemInventoryExists(var InventoryProfile: Record "Inventory Profile") ItemInventoryExists: Boolean
    begin
        with InventoryProfile do begin
            SetRange("Source Type", DATABASE::"Item Ledger Entry");
    ...
    local procedure NS_ApplyUntrackedQuantityToItemInventory(SupplyExists: Boolean; ItemInventoryExists: Boolean): Boolean
    begin
        if SupplyExists then
            exit(false);
    ...
    local procedure NS_UpdateAppliedItemEntry(var ReservEntry: Record "Reservation Entry")
    begin
    ...
    local procedure NS_CheckSupplyAndTrack(InventoryProfileFromDemand: Record "Inventory Profile"; InventoryProfileFromSupply: Record "Inventory Profile")
    begin
        if InventoryProfileFromSupply."Source Type" = DATABASE::"Item Ledger Entry" then
            NS_Track(InventoryProfileFromDemand, InventoryProfileFromSupply, false, false, InventoryProfileFromSupply.Binding.AsInteger())
        else
            NS_Track(InventoryProfileFromDemand, InventoryProfileFromSupply, false, false, InventoryProfileFromDemand.Binding.AsInteger());
    end;

    local procedure NS_CheckPlanSKU(SKU: Record "Stockkeeping Unit"; DemandExists: Boolean; SupplyExists: Boolean; IsReorderPointPlanning: Boolean): Boolean
    begin
    ...
    local procedure NS_PrepareDemand(var InventoryProfile: Record "Inventory Profile"; IsReorderPointPlanning: Boolean; ToDate: Date)
    begin
        // Transfer attributes
        if (TempSKU."Reordering Policy" = TempSKU."Reordering Policy"::Order) or
           (TempSKU."Manufacturing Policy" = TempSKU."Manufacturing Policy"::"Make-to-Order")
        then
            NS_PrepareOrderToOrderLink(InventoryProfile);
        NS_UpdatePriorities(InventoryProfile, IsReorderPointPlanning, ToDate);
    end;

    local procedure NS_DemandMatchedSupply(var FromInventoryProfile: Record "Inventory Profile"; var ToInventoryProfile: Record "Inventory Profile"; SKU: Record "Stockkeeping Unit"): Boolean
    var
    ...
    local procedure NS_ReservedForProdComponent(ReservationEntry: Record "Reservation Entry"): Boolean
    begin
    ...
    local procedure NS_ShouldInsertTrackingEntry(FromTrkgReservEntry: Record "Reservation Entry"): Boolean
    var
    ...
    local procedure NS_CloseInventoryProfile(var ClosedInvtProfile: Record "Inventory Profile"; var OpenInvtProfile: Record "Inventory Profile"; ActionMessage: Option " ",New,"Change Qty.",Reschedule,"Resched.& Chg. Qty.",Cancel)
    var
    ...
    begin
        ...
        //ProjectPro - start
        OpenInvtProfile."NS_Job No." := JobNo;
        OpenInvtProfile."NS_Job Task No." := JobTaskNo;
        //ProjectPro - end
        ...
        if ActionMessage <> ActionMessage::" " then
            if OpenInvtProfile.IsSupply then
                NS_MaintainPlanningLine(OpenInvtProfile, PlanningStageToMaintain, ScheduleDirection::Backward)
            else
                NS_MaintainPlanningLine(ClosedInvtProfile, PlanningStageToMaintain, ScheduleDirection::Backward);

        NS_Track(ClosedInvtProfile, OpenInvtProfile, false, false, OpenInvtProfile.Binding.AsInteger());
    ...
    local procedure NS_CloseDemand(var DemandInvtProfile: Record "Inventory Profile"; var SupplyInvtProfile: Record "Inventory Profile")
    begin
        NS_CloseInventoryProfile(DemandInvtProfile, SupplyInvtProfile, SupplyInvtProfile."Action Message");
    end;

    local procedure NS_CloseSupply(var DemandInvtProfile: Record "Inventory Profile"; var SupplyInvtProfile: Record "Inventory Profile"): Boolean
    begin
        NS_CloseInventoryProfile(SupplyInvtProfile, DemandInvtProfile, SupplyInvtProfile."Action Message");
        exit(SupplyInvtProfile.Next <> 0);
    end;

    local procedure NS_QtyPickedForSourceDocument(TrkgReservEntry: Record "Reservation Entry"): Decimal
    var
        WhseEntry: Record "Warehouse Entry";
    begin
    ...
    local procedure NS_CreateTempSKUForComponentsLocation(var Item: Record Item)
    var
        SKU: Record "Stockkeeping Unit";
    begin
        ...
        Item.CopyFilter("Variant Filter", SKU."Variant Code");
        if SKU.IsEmpty then
            NS_CreateTempSKUForLocation(Item."No.", ManufacturingSetup."Components at Location");
    end;

    local procedure NS_ForecastInitDemand(var InventoryProfile: Record "Inventory Profile"; ProductionForecastEntry: Record "Production Forecast Entry"; ItemNo: Code[20]; LocationCode: Code[10]; TotalForecastQty: Decimal)
    begin
        with InventoryProfile do begin
            Init;
            "Line No." := NS_NextLineNo;
    ...
    local procedure NS_SetPurchase(var PurchaseLine: Record "Purchase Line"; var InventoryProfile: Record "Inventory Profile")
    begin
        with ReqLine do begin
            "Ref. Order Type" := "Ref. Order Type"::Purchase;
    ...
    local procedure NS_SetProdOrder(var ProdOrderLine: Record "Prod. Order Line"; var InventoryProfile: Record "Inventory Profile")
    begin
        with ReqLine do begin
            "Ref. Order Type" := "Ref. Order Type"::"Prod. Order";
    ...
    local procedure NS_SetAssembly(var AsmHeader: Record "Assembly Header"; var InventoryProfile: Record "Inventory Profile")
    begin
        with ReqLine do begin
            "Ref. Order Type" := "Ref. Order Type"::Assembly;
            "Ref. Order No." := InventoryProfile."Source ID";
    ...
    local procedure NS_SetTransfer(var TransLine: Record "Transfer Line"; var InventoryProfile: Record "Inventory Profile")
    begin
        with ReqLine do begin
            "Ref. Order Type" := "Ref. Order Type"::Transfer;
    ...
    local procedure NS_SKURequiresLotAccumulation(var StockkeepingUnit: Record "Stockkeeping Unit"): Boolean
    var
    ...
    local procedure NS_FromLotAccumulationPeriodStartDate(LotAccumulationPeriodStartDate: Date; DemandDueDate: Date): Boolean
    begin
    ...
    local procedure NS_OnAfterTransToChildInvProfile(var ReservEntry: Record "Reservation Entry"; var ChildInvtProfile: Record "Inventory Profile")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnAfterDemandToInvProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; var ReservEntry: Record "Reservation Entry"; var NextLineNo: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnAfterSupplyToInvProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; var ToDate: Date; var ReservEntry: Record "Reservation Entry"; var NextLineNo: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforeSupplyToInvProfile(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; var ToDate: Date; var ReservEntry: Record "Reservation Entry"; var NextLineNo: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnAfterSetOrderPriority(var InventoryProfile: Record "Inventory Profile")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforeBlanketOrderConsumpFind(var BlanketSalesLine: Record "Sales Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforeCheckScheduleOut(var InventoryProfile: Record "Inventory Profile"; var TempStockkeepingUnit: Record "Stockkeeping Unit" temporary; BucketSize: DateFormula)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforeDemandInvtProfileInsert(var InventoryProfile: Record "Inventory Profile"; StockkeepingUnit: Record "Stockkeeping Unit")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforeMatchAttributesDemandApplicationLoop(var SupplyInventoryProfile: Record "Inventory Profile"; var DemandInventoryProfile: Record "Inventory Profile"; var SupplyExists: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnEndMatchAttributesDemandApplicationLoop(var SupplyInventoryProfile: Record "Inventory Profile"; var DemandInventoryProfile: Record "Inventory Profile"; var SupplyExists: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnStartOfMatchAttributesDemandApplicationLoop(var SupplyInventoryProfile: Record "Inventory Profile"; var DemandInventoryProfile: Record "Inventory Profile"; var SupplyExists: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforePrePlanDateApplicationLoop(var SupplyInventoryProfile: Record "Inventory Profile"; var DemandInventoryProfile: Record "Inventory Profile"; var SupplyExists: Boolean; var DemandExists: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnStartOfPrePlanDateApplicationLoop(var SupplyInventoryProfile: Record "Inventory Profile"; var DemandInventoryProfile: Record "Inventory Profile"; var SupplyExists: Boolean; var DemandExists: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforePrePlanDateDemandProc(var SupplyInventoryProfile: Record "Inventory Profile"; var DemandInventoryProfile: Record "Inventory Profile"; var SupplyExists: Boolean; var DemandExists: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforePrePlanDateSupplyProc(var SupplyInventoryProfile: Record "Inventory Profile"; var DemandInventoryProfile: Record "Inventory Profile"; var SupplyExists: Boolean; var DemandExists: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnAfterPrePlanDateSupplyProc(var SupplyInventoryProfile: Record "Inventory Profile"; var DemandInventoryProfile: Record "Inventory Profile"; var SupplyExists: Boolean; var DemandExists: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforePlanStepSettingOnStartOver(var SupplyInventoryProfile: Record "Inventory Profile"; var DemandInventoryProfile: Record "Inventory Profile"; var SupplyExists: Boolean; var DemandExists: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforeCalculatePlanFromWorksheet(var Item: Record Item; ManufacturingSetup2: Record "Manufacturing Setup"; TemplateName: Code[10]; WorksheetName: Code[10]; OrderDate: Date; ToDate: Date; MRPPlanning: Boolean; RespectPlanningParm: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnAfterInsertSafetyStockDemands(var DemandInvtProfile: Record "Inventory Profile"; xDemandInvtProfile: Record "Inventory Profile"; var TempSafetyStockInvtProfile: Record "Inventory Profile" temporary; var TempStockkeepingUnit: Record "Stockkeeping Unit" temporary; var PlanningStartDate: Date; var PlanToDate: Date)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnFindCombinationAfterAssignTempSKU(var TempStockkeepingUnit: Record "Stockkeeping Unit" temporary; InventoryProfile: Record "Inventory Profile")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnBeforePostInvChgReminder(var InventoryProfileChangeReminder: Record "Inventory Profile"; var InventoryProfile: Record "Inventory Profile"; PostOnlyMinimum: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnAfterPostInvChgReminder(var InventoryProfileChangeReminder: Record "Inventory Profile"; var InventoryProfile: Record "Inventory Profile"; PostOnlyMinimum: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure NS_OnEndOfPrePlanDateApplicationLoop(var SupplyInventoryProfile: Record "Inventory Profile"; var DemandInventoryProfile: Record "Inventory Profile"; var SupplyExists: Boolean; var DemandExists: Boolean)
    begin
    end;

    local procedure NS_AddJobDemandOnly(var InventoryProfile: Record "Inventory Profile"; var Item: Record Item; ToDate: Date; JobDemandOnly: Boolean; var JobNoFilter: Code[20]; FromDate: Date; DocumentNo: Code[20])
    var
        JobPlanLine: Record "Job Planning Line";
        LastLineNo: Integer;
        ReqLine2: Record "Requisition Line";
        JobRec: Record Job;
        JobMatPlanRec: Record "NS_Job Material Planning";
        DefDim: Record "Default Dimension";
        DimSet: Record "Dimension Set Entry";
        DimSetEntry: Integer;
        CalcJMPQty: Report "NS_Load Job Jnl from JMP";
    ...
    begin
        ...
        GLSetup.Get;
        ReqLine2.Reset;
        ReqLine2.SetRange("Worksheet Template Name", CurrTemplateName);
        ReqLine2.SetRange("Journal Batch Name", CurrWorksheetName);
        if ReqLine2.FindLast then
            LastLineNo := ReqLine2."Line No."
        else
            LastLineNo := 0;

        if JobDemandOnly then begin
            //Now get the JOB Mat Plan Record
            ...
            if JobMatPlanRec.FindFirst then begin
                repeat
                    ...
                    if ReqLine.Insert then begin
                        ...
                        // with ReqLine do begin
                        //     DefDim.Reset;
                        //     DefDim.SetRange("Table ID", DATABASE::Job);
                        //     x := 0;
                        //     DefDim.SetRange("No.", JobMatPlanRec."NS_Worksheet Job No.");
                        //     if DefDim.FindSet(false, false) then
                        //         repeat
                        //             if DefDim."Dimension Code" = GLSetup."Shortcut Dimension 1 Code" then
                        //                 x := 1;
                        //             if DefDim."Dimension Code" = GLSetup."Shortcut Dimension 2 Code" then
                        //                 x := 2;
                        //             if DefDim."Dimension Code" = GLSetup."Shortcut Dimension 3 Code" then
                        //                 x := 3;
                        //             if DefDim."Dimension Code" = GLSetup."Shortcut Dimension 4 Code" then
                        //                 x := 4;
                        //             if DefDim."Dimension Code" = GLSetup."Shortcut Dimension 5 Code" then
                        //                 x := 5;
                        //             if DefDim."Dimension Code" = GLSetup."Shortcut Dimension 6 Code" then
                        //                 x := 6;
                        //             if DefDim."Dimension Code" = GLSetup."Shortcut Dimension 7 Code" then
                        //                 x := 7;
                        //             if DefDim."Dimension Code" = GLSetup."Shortcut Dimension 8 Code" then
                        //                 x := 8;
                        //             ValidateShortcutDimCode(x, DefDim."Dimension Value Code");
                        //             Modify;
                        //         //Need Dimension Tracking for Req Wksht

                        //         until DefDim.Next = 0;
                        // end;
                        //PRJ-711.AS.1.0 07MAY2021 - END Commented code
                        //ProjectPro - start
                        if JobMatPlanRec."NS_Location Code" <> '' then
                            ReqLine."Location Code" := JobMatPlanRec."NS_Location Code"
                        else
                ...
                until JobMatPlanRec.Next = 0;
            end;
        end else begin
            Item.CalcFields("Qty. on Job Order");
            if Item."Qty. on Job Order" <> 0 then begin
                //Now loop through the job Planning lines and add the items to
                JobPlanLine.Reset;
                JobPlanLine.SetRange(Type, JobPlanLine.Type::Item);
                JobPlanLine.SetRange("No.", Item."No.");
                if JobNoFilter <> '' then
                    JobPlanLine.SetFilter("Job No.", JobNoFilter);
                if JobPlanLine.FindFirst then begin
                    repeat
                        ...
                        ReqLine."Line No." := LastLineNo;
                        ReqLine.Type := ReqLine.Type::Item;
                        ReqLine.Validate("No.", JobPlanLine."No.");
                        ReqLine."Variant Code" := JobPlanLine."Variant Code";
                        ReqLine."Location Code" := JobSU."NS_Job Mat'l Planning Location";
                        ReqLine."Action Message" := ReqLine."Action Message"::New;
                        ...
                        ReqLine."Location Code" := JobSU."NS_Job Mat'l Planning Location";
                        //PRJ-130.MS.1.0 start
                        if JobPlanLine."NS_Vendor No." <> '' then
                            ReqLine."Vendor No." := JobPlanLine."NS_Vendor No."
                        else
                            ReqLine."Vendor No." := Item."Vendor No.";
                        //ReqLine."Vendor No.":=Item."Vendor No."; comment
                        //PRJ-130.MS.1.0 end
                        ReqLine."Vendor Item No." := Item."Vendor Item No.";
                        ReqLine."NS_Segment Code" := JobMatPlanRec."NS_Segment Code";//TM-10.AM.1.0
                        ReqLine."NS_JMP Details" := JobMatPlanRec."NS_Details";  //PRJCTPR-256.JS.1.0 15DEC2023
