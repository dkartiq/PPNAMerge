codeunit 14021198 "NS_CopyJob Card"
{

    //PRJ-1616.AS.1.0 New CodeUnit
    trigger OnRun()
    begin
    end;
    ...
    begin
        ...
        NS_AssemblyBOM2InsertWhileCopyJob(SourceJob, TargetJob);//PRJ-1616.AS.1.0
    end;
        ...
        if SourceJobPlanningLine.FindSet then
            repeat
                with TargetJobPlanningLine do begin
                    ...
                    Validate("Line No.", SourceJobPlanningLine."Line No.");
                    ...
                    "Ledger Entry Type" := "Ledger Entry Type"::" ";
                     TargetJobPlanningLine."NS_ProgessBillingNo" := ''; //PRJCTPR-191.DK.1.0 12Sep2023
                    Insert(true);
    ...
    local procedure NS_AssemblyBOM2InsertWhileCopyJob(SourceJob: Record Job; var TargetJob: Record Job)
    var
        ...
        AssBOMComp3: Record "NS_Assembley BOM Components";
        jpl: Record "Job Planning Line";
        jobsrec: Record Job;//PRJ-1616.AS.1.0
    begin
        //PRJ-1616.AS.1.0 start
        if jobsrec.get(TargetJob."No.") then begin
            jobsrec.NS_CopyJob := true;
            jobsrec.Modify();
        end;
        //PRJ-1616.AS.1.0 end

        AssBOMComp1.Reset();
        AssBOMComp1.SetRange("NS_Job No.", TargetJob."No.");
        if AssBOMComp1.FindSet() then begin
            AssBOMComp1.DeleteAll();
        end;

        jpl.Reset();
        jpl.SetRange("Job No.", SourceJob."No.");
        if jpl.FindSet() then
            repeat
                AssBOMComp1.Reset();
                AssBOMComp1.SetRange("NS_Job No.", jpl."Job No.");
                AssBOMComp1.SetRange("NS_Job Task No.", jpl."Job Task No.");
                AssBOMComp1.SetRange("NS_Ref. JPL Line No.", jpl."Line No.");
                if AssBOMComp1.FindSet() then
                    repeat
                        AssBOMComp2.Init();
                        AssBOMComp2."NS_Job No." := TargetJob."No.";
                        AssBOMComp2."NS_Job Task No." := AssBOMComp1."NS_Job Task No.";

                        AssBOMComp3.Reset();
                        AssBOMComp3.SetRange("NS_Job No.", TargetJob."No.");
                        AssBOMComp3.SetRange("NS_Job Task No.", jpl."Job Task No.");
                        if AssBOMComp3.FindLast() then
                            AssBOMComp2."NS_Line No." := AssBOMComp3."NS_Line No." + 1000
                        else
                            AssBOMComp2."NS_Line No." := 1000;

                        AssBOMComp2."NS_SourceASMBOMLineNo." := AssBOMComp1."NS_Line No.";

                        AssBOMComp2.NS_Type := AssBOMComp1.NS_Type;
                        AssBOMComp2."NS_No." := AssBOMComp1."NS_No.";
                        AssBOMComp2."NS_Assembly BOM" := AssBOMComp1."NS_Assembly BOM";
                        AssBOMComp2."NS_Description New" := AssBOMComp1."NS_Description New";
                        AssBOMComp2."NS_Unit Cost" := AssBOMComp1."NS_Unit Cost";
                        AssBOMComp2.NS_Level := AssBOMComp1.NS_Level;
                        AssBOMComp2."NS_Main Item" := AssBOMComp1."NS_Main Item";
                        AssBOMComp2."NS_Assembly BOM" := AssBOMComp1."NS_Assembly BOM";
                        AssBOMComp2."NS_Item Type" := AssBOMComp1."NS_Item Type";
                        AssBOMComp2."NS_Quantity Per" := AssBOMComp1."NS_Quantity Per";
                        AssBOMComp2."NS_Unit of Measure Code" := AssBOMComp1."NS_Unit of Measure Code";
                        AssBOMComp2."NS_Quantity of Assembly Item on Job" := AssBOMComp1."NS_Quantity of Assembly Item on Job";
                        AssBOMComp2."NS_Expected Quantity" := AssBOMComp1."NS_Expected Quantity";
                        AssBOMComp2."NS_Ref. JPL Line No." := AssBOMComp1."NS_Ref. JPL Line No.";
                        AssBOMComp2."NS_Ref. JPL Parent Item No." := AssBOMComp1."NS_Ref. JPL Parent Item No.";
                        AssBOMComp2."NS_Ref. ASMBOM Line No." := AssBOMComp1."NS_Ref. ASMBOM Line No.";
                        AssBOMComp2."NS_JPL DocNo" := AssBOMComp1."NS_JPL DocNo";
                        AssBOMComp2.Insert();
                    until AssBOMComp1.Next() = 0;
            until jpl.Next() = 0;

        //PRJ-1616.AS.1.0 start
        if jobsrec.get(TargetJob."No.") then begin
            jobsrec.NS_CopyJob := false;
            jobsrec.Modify();
        end;
        //PRJ-1616.AS.1.0 end
    end;
