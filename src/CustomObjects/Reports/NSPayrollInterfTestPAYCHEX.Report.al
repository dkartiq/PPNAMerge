report 14021377 "NS_Payroll Interf. TestPAYCHEX"
{
    // version PPNA11.00
    ...
    // +------------------------------------------------------------
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/NSPayroll Interf. Test - PAYCHEX.rdl';
        ...
        dataitem("Payroll Interface Jnl Batch"; "NS_Payroll Interface Jnl Batch")
        {
            ...
            dataitem("Integer"; "Integer")
            {
                ...
                dataitem("Payroll Interface Jnl Line"; "NS_Payroll Interface Jnl Line")
                {
                    ...
                    column(Job_Journal_Line_Work_Date; "NS_Work Date")
                    {
                    }
                    ...
                    begin
                        ...
                        if STRLEN("NS_Employee No.") > 6 then
                            AddError(STRSUBSTNO(Text004, FIELDCAPTION("NS_Employee No."), 6));
                        if STRLEN("NS_Employee Name") > 25 then
                            AddError(STRSUBSTNO(Text004, FIELDCAPTION("NS_Employee Name"), 25));
                        if STRLEN("NS_Override Dept.") > 6 then
                            AddError(STRSUBSTNO(Text004, FIELDCAPTION("NS_Override Dept."), 6));
                        if STRLEN("NS_Job No.") > 12 then
                            AddError(STRSUBSTNO(Text004, FIELDCAPTION("NS_Job No."), 12));
                        if STRLEN(NS_Shift) > 1 then
                            AddError(STRSUBSTNO(Text007, FIELDCAPTION(NS_Shift)));
                        if "NS_D/E Type" = "NS_D/E Type"::Earning then begin
                            ...
                            if STRLEN(HumanResourcesSetup."NS_Earning Code Identifier") > 1 then
                                AddError(STRSUBSTNO(Text008, HumanResourcesSetup.FIELDCAPTION("NS_Earning Code Identifier"), HumanResourcesSetup.TABLECAPTION, 1));
                        end else begin
                            ...
                            if STRLEN(HumanResourcesSetup."NS_Deduction Code Identifier") > 1 then
                                AddError(STRSUBSTNO(Text008, HumanResourcesSetup.FIELDCAPTION("NS_Deduction Code Identifier"), HumanResourcesSetup.TABLECAPTION, 1));
                        end;
                        if STRLEN("NS_D/E Code") > 2 then
                            AddError(STRSUBSTNO(Text007, FIELDCAPTION("NS_D/E Code")));
                        if ("NS_D/E Code" = '') or
                           ("NS_D/E Code" = ' ') or
                           ("NS_D/E Code" = '  ') then
                            AddError(STRSUBSTNO(Text001, FIELDCAPTION("NS_D/E Code")));
                        if NS_Rate > 999999999 then
                            AddError(STRSUBSTNO(Text006, FIELDCAPTION(NS_Rate), 999999999));
                        if NS_Hours > 99999999 then
                            AddError(STRSUBSTNO(Text006, FIELDCAPTION(NS_Hours), 99999999));
                        ...
                        if NS_Amount > 999999999 then
                            AddError(STRSUBSTNO(Text006, FIELDCAPTION(NS_Amount), 999999999));
                        if "NS_Sequence No." > 1 then
                            AddError(STRSUBSTNO(Text006, FIELDCAPTION("NS_Sequence No."), 9));
                        if STRLEN("NS_Override Division") > 6 then
                            AddError(STRSUBSTNO(Text004, FIELDCAPTION("NS_Override Division"), 6));
                        if STRLEN("NS_Override Branch") > 6 then
                            AddError(STRSUBSTNO(Text004, FIELDCAPTION("NS_Override Branch"), 6));
                        if STRLEN("NS_Override State") > 2 then
                            AddError(STRSUBSTNO(Text004, FIELDCAPTION("NS_Override State"), 2));
                        if STRLEN("NS_Override Local") > 10 then
                            AddError(STRSUBSTNO(Text004, FIELDCAPTION("NS_Override Local"), 10));
                        if STRLEN("NS_State/Local Misc. Field") > 1 then
                            AddError(STRSUBSTNO(Text007, FIELDCAPTION("NS_State/Local Misc. Field")));
                        if STRLEN("NS_Rate No.") > 1 then
                            AddError(STRSUBSTNO(Text007, FIELDCAPTION("NS_Rate No.")));
