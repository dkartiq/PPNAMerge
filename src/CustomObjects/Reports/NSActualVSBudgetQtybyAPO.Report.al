report 14021156 "NS_Actual vs Budget Qty by APO"
{
    // version PPNA11.00
    ...
    dataset
    {
        dataitem(Job; Job)
        {
            ...
            dataitem("Sub-Levels"; Job)
            {
                ...
                {
                    DataItemLink = "Job No." = FIELD("No.");
                    DataItemTableView = SORTING("Job No.", "NS_Entry Type", "NS_Activity Code", "NS_Process Code", "NS_Operation Code", "NS_Section Code", "NS_Cost Category", Type, "No.", "Variant Code") ORDER(Ascending);//PRJ-750.AS.1.0 Added section
                ...
                {
                    DataItemLink = "Job No." = FIELD("No.");
                    DataItemTableView = SORTING("Job No.", "Entry Type", "NS_Activity Code", "NS_Process Code", "NS_Operation Code", "NS_Section Code", "NS_Job Cost Category", "NS_Job Revenue Category", Type, "No.", "Resource Group No.", "Posting Date") ORDER(Ascending);//PRJ-750.AS.1.0
            ...
            dataitem("Page Header"; "Integer")
            {
                ...
                column(ActualQtyCaption; ActualQtyLbl)
                {
                }
                column(BudgetedQtyCaption; BudgetedQtyLbl)
                {
                }
                column(PctOfBudUsedCaption; PctOfBudUsedLbl)
                {
                }
                column(QtyVarianceCaption; QtyVarianceLbl)
                {
                }
                ...
                dataitem(JobAnalysisBuffer1Loop; Integer)
                {
                    ...
                    dataitem(JobAndSubJob; Integer)
                    {
                        ...
                        dataitem(JobDetail; Integer)
                        {
                            ...
                            column(DetailUnitOfMeasure; JobReportBuffer1."NS_Unit Of Measure")
                            {
                            }
                            column(DetailActualCostQty; JobReportBuffer1."NS_Actual Cost Qty.")
                            {
                            }
                            column(DetailBudgetedCostQty; JobReportBuffer1."NS_Budgeted Cost Qty.")
                            {
                            }
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                field("NS_Cost Category Type"; CategoryType)
                {
                    Caption = 'Cost Category Type';
                    ApplicationArea = all;
                }
    ...
    var
        ...
        CategoryType: Option " ",Labor,Material,Equipment,SubContract,Manufacturing,Overhead,Miscellaneous;
        NoHeadingLbl: Text[3];
        ...
        MustSelectCategoryType: Label 'There must be a category type selected.';
        ReportTitle: Label 'Actual vs Budget Quantity for Job';
        PageLbl: Label 'Page';
        JobFiltersLbl: Label 'Job Filters:';
        DescriptionLbl: Label 'Description';
        NoLbl: Label 'No.';
        UOMLbl: Label 'UOM';
        ActualQtyLbl: Label 'Actual Quantity';
