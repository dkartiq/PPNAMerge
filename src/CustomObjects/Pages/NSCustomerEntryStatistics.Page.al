page 14021474 "NS_Customer Entry Statistics"
{
    // version NAVW113.00
    //SPLN1.00 2019-02-07 DMT Copy of Page 302
    // +---------------------------------------------------------------------------------------------
    // +ProjectPro
    // +  - Added field(s):
    // +
    ...
    // +
    // +  - Modification(s):
    // +     - OnOpenPage - Read PP_SalesSetup record
    // +     - OnAfterGetRecord - Set Retention Ledger Code filtering if using retention - five places
    // +                        - Modify current key setting on CustLedgEntry2 to include Retention Ledger Code
    // +-----------------------------------------------------------------------------------------------

    Caption = 'PP Customer Entry Statistics';
    Editable = false;
    LinksAllowed = false;
    PageType = Card;
    ...
    begin
        ...
        for j := 1 to 6 do begin
            ...
            //ProjectPro - start
            IF NOT PP_SalesSetup."NS_Sales Retention Inactive" THEN
                CustLedgEntry[j].SETRANGE("NS_Retention Ledger Code", PP_SalesSetup."NS_Normal Customer Ledger No.");
            //ProjectPro - end
        ...
        //ProjectPro - start
        IF NOT PP_SalesSetup."NS_Sales Retention Inactive" THEN
            CustLedgEntry2.SETRANGE("NS_Retention Ledger Code", PP_SalesSetup."NS_Normal Customer Ledger No.");
        //ProjectPro - end        
        ...
        for i := 1 to 3 do begin
            ...
            DtldCustLedgEntry2.SETCURRENTKEY("Customer No.", "Posting Date", "Initial Entry Global Dim. 1", "Initial Entry Global Dim. 2", "NS_Retention Ledger Code");
            //ProjectPro - end
            ...
            //ProjectPro - start
            IF NOT PP_SalesSetup."NS_Sales Retention Inactive" THEN
                CustLedgEntry2.COPYFILTER("NS_Retention Ledger Code", DtldCustLedgEntry2."NS_Retention Ledger Code");
            //ProjectPro - end          
                ...
                repeat
                    ...
                    then
                        if CustLedgEntry2."Closed at Date" > CustLedgEntry2."Posting Date" then
                            NS_UpdateDaysToPay(CustLedgEntry2."Closed at Date" - CustLedgEntry2."Posting Date")
                        else
                            if CustLedgEntry2."Closed by Entry No." <> 0 then begin
                                if CustLedgEntry3.Get(CustLedgEntry2."Closed by Entry No.") then
                                    NS_UpdateDaysToPay(CustLedgEntry3."Posting Date" - CustLedgEntry2."Posting Date");
                            end else begin
                                CustLedgEntry3.SetCurrentKey("Closed by Entry No.");
                                CustLedgEntry3.SetRange("Closed by Entry No.", CustLedgEntry2."Entry No.");
                                //ProjectPro - start
                                IF NOT PP_SalesSetup."NS_Sales Retention Inactive" THEN
                                    CustLedgEntry3.SETRANGE("NS_Retention Ledger Code", CustLedgEntry2."NS_Retention Ledger Code");
                                //ProjectPro - end                    
    ...
    local procedure NS_UpdateDaysToPay(NoOfDays: Integer)
    begin
