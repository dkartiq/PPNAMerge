page 14021456 "NS_Job Forecast Work by Seg"
{
    // version PPNA11.00
    ...
    //JD-48.AS.1.0 Added more code
    //JD-48.AS.2.0 Added code
    Caption = 'Job Forecast Worksheet by Segment';
        ...
        area(content)
        {
            ...
            group(Control1100773006)
            {
                ...
                field(CurrSegCode; CurrSegCode)
                {
                    ...
                    begin
                        CurrPage.SAVERECORD;
                        COMMIT;
                        SegmentHeader."NS_Job No." := CurrentJobNo;
                        SegmentHeader."NS_Segment Code" := CurrSegCode;
                        SegmentHeader_L1.Reset();
                        SegmentHeader_L1.SetRange("NS_Job No.", CurrentJobNo);
                        if PAGE.RUNMODAL(14021400, SegmentHeader_L1) = ACTION::LookupOK then begin
                            SegName := '';
                            CurrSegCode := SegmentHeader_L1."NS_Segment Code";
                            SegName := SegmentHeader_L1."NS_Segment Name";
                            //    end;
                            //end;
                        end;
                        NS_ListUpdate;
                    ...
                    trigger OnValidate();
                    var
                        SegmentHeader_L2: Record "NS_Job Takeoff Segments";
                    begin
                        SegName := '';
                        if SegmentHeader."NS_Segment Code" > '' then begin
                            SegmentHeader_L2.Reset();
                            SegmentHeader_L2.SetRange("NS_Job No.", CurrentJobNo);
                            SegmentHeader_L2.SetRange("NS_Segment Code", SegmentHeader."NS_Segment Code");
                            if SegmentHeader_L2.FindFirst then begin
                                SegName := SegmentHeader_L2."NS_Segment Name";
                            end;
                            CurrPage.SAVERECORD;
                        end;
                        NS_ListUpdate;
            ...
            repeater(CSWorkSheet)
            {
                ...
                field("NS_Segment Code"; Rec."NS_Segment Code")
                {
                    ...
                    trigger OnValidate()
                    var
                        Seghdr: Record "NS_Job Takeoff Segments";
                    begin
                        Seghdr.SetRange("NS_Job No.", CurrentJobNo);
                        Seghdr.SetRange("NS_Segment Code", Seghdr."NS_Segment Code");
                        if Seghdr.FindFirst then begin
                            SegName := SegmentHeader."NS_Segment Name";
                        end;
                    end;
                }
                ...
                field("NS_Units Complete"; Rec."NS_Units Complete")
                {
                    ...
                    trigger OnValidate();
                    begin
                        NS_UnitsCompleteOnAfterValidate;
                        NS_SetBillDate;
                        NS_SetStatusDate;
                    end;
                }
    ...
    actions
    {
        area(processing)
        {
            ...
            action(NS_Print)
            {
                ...
                Promoted = true;
                PromotedCategory = Process;
                ToolTip = 'Print the job forecast worksheet.';//JD-48.AS.1.0 31OCT2020 Also Changed this code acc to new

                trigger OnAction();
                var
                    ReportJobForecast: Record "NS_Job Forecast by Seg code";
                begin
                    if CurrentJobNo > '' then begin
                        if AsOfDateFilter > 0D then begin
            ...
            action(NS_Post)
            {
                ...
                PromotedCategory = Process;
                PromotedIsBig = true;
                ToolTip = 'Post the job forecast worksheet by Segment';
                ...
                begin
                    JobFWBySEG.Reset;
                    JobFWBySEG.SetRange("NS_Job No.", Rec."NS_Job No.");
                    JobFWBySEG.SetFilter("NS_Segment Code", '%1', '');
                    if JobFWBySEG.FindFirst then
                        Error('You are not allowed to post Job Forecast worksheet with blank segment code');
                    NS_PostLines(CurrentJobNo, AsOfDateFilter, NextBillDate);
                end;
                //JD-48.AS.1.0 - end
            }
            action(NS_ProjectionList)
            {
                ApplicationArea = All;
            ...
            action(NS_GetProjections)
            {
                ...
                trigger OnAction();
                var
                    ForecastRecs: Record "NS_Job Forecast by Seg code";
                begin
                    ForecastRecs.RESET();
                    ...
                    if CurrSegCode > '' then
                        ForecastRecs.SETRANGE("NS_Segment Code", CurrSegCode);
            ...
            action(NS_CalcExpectedReceiptCosts)
            {
                ApplicationArea = All;
                Caption = 'Calc Expected Receipt Costs';
                ...
                trigger OnAction();
                var
                    JobForecastLines: Record "NS_Job Forecast by Seg code";
                begin
                    if (CurrentJobNo > '') and (NextBillDate > 0D) then begin
                        with JobForecastLines do begin
                            RESET;
                            SETCURRENTKEY("NS_Job No.", "NS_Segment Code", "NS_Job Task No.", NS_Posted);
                                ...
                                repeat
                                    if "NS_Calc Expected Receipt Costs" then begin
                                        ...
                                        if "NS_PO Expected Receipt Cost" > 0 then begin
                                            ...
                                            NS_GetJobPlanningLineAndBudget("NS_Job No.", "NS_Job Task No.", JobPlanningLineBudget, TotalTaskBudget, "NS_Segment Code");
                                            ...
                                            NS_GetJobPlanningLineAndBudget(Rec."NS_Job No.", "NS_Job Task No.", JobPlanningLineBudget, TotalBudget, "NS_Segment Code");
                                            TotalCostsUsed := 0;
                                            if Job.GET(Rec."NS_Job No.") then begin
                                                Job.SETRANGE("NS_Job Task No. Filter", "NS_Job Task No.");
            ...
            action(NS_JFWbySegmentlist)
            {
                ApplicationArea = All;
                Caption = 'JFW by Segment List';
                Image = ListPage;
                Promoted = true;
                PromotedCategory = Process;
                PromotedIsBig = true;
                ToolTip = 'View the Job Forecast by Segment list';

                trigger OnAction();
                var
                    JFWSeglist: Page "NS_Job Forecast Work bySeglist";
                    JFWSeg: record "NS_Job Forecast by Seg code";
                begin
                    JFWSeg.Reset;
                    JFWSeg.SetRange("NS_Job No.", "NS_Job No.");
                    JFWSeglist.SetTableView(JFWSeg);
                    JFWSeglist.NS_SetJobNo("NS_Job No.");
                    JFWSeglist.RunModal;
                    Clear(JFWSeglist);
                end;
            }
