page 14021206 "NS_Committed Line List"
{
    // version PPNA11.00
    ...
    // +  - www.gemko.com
    //PRJ-321.MS.1.0 added code for commited cost calculation
    // +------------------------------------------------------------
    ...
    begin
        //RESET;
        SetFilter("Document Type", '%1|%2', "Document Type"::Order, "Document Type"::"Return Order");//PRJ-321.MS.1.0
        if JobNo > '' then
            SETRANGE("Job No.", JobNo);
        if SubcontractNo > '' then
            SETFILTER("NS_Subcontract No.", SubcontractNo);
        if "Document Type" = "Document Type"::"Return Order" then //PRJ-321.MS.1.0
            "NS_Committed Amount" := -"NS_Committed Amount";  //PRJ-321.MS.1.0
    end;
